...
COMMON minis, dscmin2,dscmax2,dscmin3,dscmax3,scdp,scdp1,scdp2,scdp3,scdp4
  ...
  re=6371.2 & deltab=0.2 &  absmin=0.0 & absmax=0.0
  dscmin2=0.0 & dscmax2=0.0 & dscmin3=0.0 & dscmax3=0.0
  
  delt = 0.5 &  fdelt = 0.5 & isc=1
...
; DHT AND WALEN TEST Parameters
  ...
  if ( (whatindex eq 's') or $
     ...
     hclwalendhtbound2,itest,smo,tmin,tmax,delt,fdelt,$
  ...
  if whatindex eq 'p' then begin
     ...
     psname=STRCOMPRESS('cl'+datestrn+':'+string(ih1,'(i2.2)')$
                         +string(im1,'(i2.2)')$
                         +'_'+string(ih2,'(i2.2)')+string(im2,'(i2.2)')$
                         +'sc'+string(isc,'(i1.1)')+'db'$
                         +string(deltab,'(f5.2)')+ttwal,/REMOVE_ALL)
     set_plot,'ps'
     device,/color,bits=8
