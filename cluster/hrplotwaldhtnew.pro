...
;   TEMPERATURE
    ...
    xyouts, min, bmin-0.25*(bmax-bmin), 'search t and dt : '$
     +string(delt,'(f5.2)')$
    +' ; '+string(fdelt,'(f5.2)'),charsize=0.8
    ...
    xyouts, max, bmin-0.25*(bmax-bmin), 'slope range for W:'$
                 +string(slwal0,'(f5.2)')$
                            +' ; '+string(slwal1,'(f5.2)'),charsize=0.8
    endif

    if itest ne 1 then begin
    xyouts, max, bmin-0.4*(bmax-bmin), 'slope range for HT:'$
    +string(slht0,'(f5.2)')$
                            +';'+string(slht1,'(f5.2)'),charsize=0.8
    endif
