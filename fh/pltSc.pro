...
; Version 2
; This version incorporates the non-uniform grid in rho and the
; new magnetopause boundary definition.
; by Fred Hall, IV
; 18 September 2000
...
mpccrho=[mpccrho,0.0] & mpccphi=[mpccphi,0.0]
print
print
print
...
;   if (S(i,j) eq -1e5) then S(i,j) = noplt
    if (S(i,j) eq TsySinit) then S(i,j) = noplt
    if ((S(i,j) lt 0.0) AND (S(i,j) ne noplt)) then begin $
      print, 'i = ',i
      print, 'j = ',j
      print, 'S(i,j) = ',S(i,j)
    endif
...
for kmpc=0,nphi do $
  if (stcross(kmpc) ne nrho) then begin $
      ir=stcross(kmpc)+1
      if ir lt nrho then S(ir,kmpc) = 1.5*S(ir-1,kmpc)
      ir=ir+1
      if ir lt nrho then S(ir,kmpc) = 2.*S(ir-1,kmpc)
      ir=ir+1
      if ir lt nrho then S(ir,kmpc) = 2.*S(ir-1,kmpc)
      ir=ir+1
      for i=ir,nrho do S(i,kmpc)=100.
  endif
...
; /T3D,/NOERASE,/SAVE $
  ...
  XSTYLE=1,YSTYLE=1, $
  LEVELS=[0.01,0.1,0.3,1.,3.], $
  C_LABELS=[0,1,0,1,1], $
  C_LINESTYLE=[0,3,0,2,0], $
  C_THICK=[1,3,1,1,1]
