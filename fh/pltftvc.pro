...
for i=0,nrho do begin
  for j=0,nphi do begin
    if (ftv(i,j) eq Tsyftvinit) then ftv(i,j) = noplt
    if ((ftv(i,j) lt 0.0) AND (ftv(i,j) ne noplt)) then begin $
      print, 'i = ',i
      print, 'j = ',j
      print, 'ftv(i,j) = ',ftv(i,j)
    endif
  endfor
endfor
for kmpc=0,nphi do $
  if (stcross(kmpc) ne nrho) then begin $
      ir=stcross(kmpc)+1
      if ir lt nrho then ftv(ir,kmpc) = 1.3*ftv(ir-1,kmpc)
      ir=ir+1
      if ir lt nrho then ftv(ir,kmpc) = 2.*ftv(ir-1,kmpc)
      ir=ir+1
      for i=ir,nrho do ftv(i,kmpc)=2.5*ftv(i-1,kmpc)
  endif
;ftv=smooth(ftv,3)
...
; /T3D,/NOERASE,/SAVE $
  TITLE='Tsyganenko T96_01 Flux Tube Volume Distribution', $
  XTITLE='x (R!DE!N)',YTITLE='y (R!DE!N)', $
  XRANGE=[-45,15],YRANGE=[-30,30], $
  XSTYLE=1,YSTYLE=1, $
  LEVELS=[0.032,0.32,1.,3.2,10.], $
  C_LABELS=[1,1,1,1,1], $
