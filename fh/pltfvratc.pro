...
;   pfunc(i,j)=drho(i)*fvr(i,j)/rho(i)
    pfunc(i,j)=drho(i)*drho(i)*fvr(i,j)/(rho(i)*rho(i))
;   pfunc(i,j)=rho(i)*rho(i)*fvr(i,j)/(drho(i)*drho(i))
...
if keyword_set(ps) then begin
  ...
  device,filename='pltfvratc.ps'
endif

; Generate the plot

polar_contour,transpose(pfunc),phi,rho, $
; /T3D,/NOERASE,/SAVE $
  TITLE='Tsyganenko T96_01 Flux Tube Volume Ratio', $
  XTITLE='x (R!DE!N)',YTITLE='y (R!DE!N)', $
  XRANGE=[-45,20],YRANGE=[-30,30], $
  XSTYLE=1,YSTYLE=1, $
  LEVELS=[0.01,0.1,1.0,10.0,100.0], $
  C_LABELS=[0,1,0,1,0], $
  C_THICK=[1.0,4.0,1.0,4.0,1.0]
