...
function chapman, z, sza, scaleheight, shadow=shadow
...
; NOTE the file chapman.pro does the same thing, but uses a different
...
w=x*(1.-sin(ss))
sw=sqrt(w)
zmin=80.  ; altitude that defines the shadow (0 for surface shadow)
...
if n180 gt 0 then begin
    ...
    y=(re+v)/scaleheight
    chap(i180)=sqrt(0.5*!pi*y)*(1.+.375/y)* $
               (1.+erf(-sqrt(y*.5)/tan(ss(i180))))
