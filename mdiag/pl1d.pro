; program to plot the 1D MHD data
  ...
  nt=101 & nx=23
  ind=0
  xr=0.1 & rr=xr & pr=xr & bxr=xr & byr=xr & vxr=xr & vyr=xr
  ...
  x=fltarr(nx) & rho=fltarr(nt,nx) & p=rho & bx=rho & by=rho & vx=rho & vy=rho
  tarr=fltarr(nt)
  ...
  while not eof(8) do begin
    ...
    for k=0, nx-1 do begin
      readf, 8, xr,rr,pr,bxr,byr,vxr,vyr
      x(k)=xr & rho(i,k)=rr & p(i,k)=pr & bx(i,k)=bxr & by(i,k)=byr
      vx(i,k)=vxr & vy(i,k)=vyr
    endfor
    i = i+1
  endwhile
  close, 8
 ...
 menu:
  ...
  if withps eq 'n' then WINDOW, XSIZE=550, YSIZE=350, $
                                TITLE='One-Dimensional MHD'      

  if anim eq 'y' then begin
      xanimate,set=[550,350,(ntot+1),0]
      ind=-1
    animate:
      ind=ind+1
  endif
