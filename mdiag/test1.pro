...
;----------------------------------------
  ...
  tsmin= fltarr(20) & tsmax=tsmin & tscrit=25.0 & nscrit=16.0
  ishade='y' & shading='o'
...
'Satellite velocity is relative to restframe of the magnetosphere (at xmin)!!!'
  ...
  if whatindex eq 'i' then begin
     ...
     if (shading ne 'f') and (shading ne 'o') then nscrit=float(shading)
  endif
  ...
  if ( (whatindex ne '') and (whatindex ne 'a') and (whatindex ne 'm') $
     ...
     and (whatindex ne 'q') and (whatindex ne 'i') )    then begin
    index=fix(whatindex)
...
;       !Y.RANGE=[ymin,ymax]
   ...
   if ishade eq 'y' then begin
     ...
     if (rho1(iend-1) gt tscrit) then begin
        tsmax(inds)=t(i) & inds=inds+1 & endif
     indstot=inds-1
...
;	ytick_get=vv
	...
	plot, t, vx1, $
	   yrange=[bmin,bmax], $
	   xstyle=1,ystyle=1,xtickname=names, line=1,/noerase
	oplot, t, -vy1, line=2
	oplot, t, vz1, line=0
        yt1=bmin+0.73*delb    
        yt1a=bmin+0.60*delb    
...
dx=1.*vscale*dxrange/dpx*dpy/psxsize*psysize
dy=1.*vscale*dyrange

for i=istart,iend do begin
 ox=t(i) 
 dx=-vx1(i)*vscale/dpx*dpy/psxsize*psysize
; dy=-(vy1(i)-170.)*vscale
 dy=-(vy1(i)-0.)*vscale
