...
;  c=1./sqrt(2.)  
  ...
  gdiss=fltarr(nx,3) & gdisp=fltarr(nx,3) & unity=fltarr(nx)
  unity=0.*unity+1.
...
; first page
  ...
  if (contin eq '' or contin eq 'y') then begin
    ...
    !P.THICK=1  
    oplot, theta, unity,linestyle=0
         xpos=1.02
         ypos=amin+0.9*(amax-amin)
         ypos1=amin+0.5*(amax-amin)
	 xyouts,xpos,ypos,'c = '+string(c,'(f5.3)')
