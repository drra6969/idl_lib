...
; tc, ts =elementary computation and communication times
; nr = number of gridpoints along np to plot f(np)
;-------------
; PARAMETER:
;-------------
  npmax=1000.0 & dnp=(npmax-2.)/(nr-1.) & np=2.0+findgen(nr)*dnp 
; npmax= max no of processor (np - axis)
; dnp= gridseparation along np
; np= plotting variable
; f=efficiency, alp=alpha(->domaindecomposition)
...
; The following addresses a correction to the efficiency 
...
; first page
  ...
  if (contin eq '' or contin eq 'y') then begin
    ...
    plot_io, np, trat1,$
        title='Time ratio, T_ctot/T_tot',xtitle='Np',$
...
;    amax = (max([tot1(tt1),tot2(tt2),tot3(tt3)]))
    amax = 80
    amin = (min([tot1,tot2,tot3]))
    plot_io, np, tot1,$
        title='Total Time Ttot',xtitle='Np',$
        font=3, yrange=[amin, amax], ystyle=1, line=0
    plot_io, np, tot2, font=3, yrange=[amin, amax], ystyle=1, line=1
    plot_io, np, tot3, font=3, yrange=[amin, amax], ystyle=1, line=2

    !P.POSITION=[0.2,0.05,0.7,0.3]
;    amax = (max([ttot1(tt1),ttot2(tt2),ttot3(tt3)]))
    amax = 20
    amin = (min([ttot1,ttot2,ttot3]))
    plot_io, np, ttot1,$
        title='Time ratio, Ttot/Ttheory',xtitle='Np',$
        font=3, yrange=[amin, amax], ystyle=1, line=0
    plot_io, np, ttot2, font=3, yrange=[amin, amax], ystyle=1, line=1
    plot_io, np, ttot3, font=3, yrange=[amin, amax], ystyle=1, line=2
