; START OF MAIN PROGRAM
  ...
  readu, 8, nx
  x=fltarr(nx,/NOZERO) & f=fltarr(nx,/NOZERO) & fexact=f
  ...
  fexact(*)=farr(*,itot)
  sum=0.
  for j=0,nx-1 do  sum = sum + ( fexact(j)-farr(j,itot-1) )^2
  rms = sqrt(sum/nx)
...
;          title='Temperature',/noerase,$
	...
	xyouts,xpos,ypos1,'rms  = '+string(rms,'(f8.6)') 
    endif
