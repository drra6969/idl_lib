...
; READ INPUT DATA OF DIMENSION NX, NY
   read2dfk, g1,g2,g3,h1,h2,h3, bx,by,bz, vx,vy,vz, rho,u,res,fnumber
...
jande, nx,ny,g1,h1,bx,by,vx,vy,res,jz,ez
      ...
      ex=-(vy*bz-vz*by) + res*jx & ey=-(vz*bx-vx*bz) + res*jy
...
asymval, nx,ny,vzpro,vz1,vz2,vzav  
 print, 'vz_msp_orig = ', vz1,'  vz_msh_orig = ',vz2,'   vzav = ',vzav
   ...
   f1=ex(*,0:ny-2)  & ex(*,0:ny-2)=shift(f1,0,nsh)  ;& ez(*,ny-1)=ez(*,0)
   f1=ey(*,0:ny-2)  & ey(*,0:ny-2)=shift(f1,0,nsh)  ;& ez(*,ny-1)=ez(*,0)
...
;     vzpro(*,ny-1)=vzpro(*,0)
   ...
   print, 'X:', x(0),x(nx-1)
   print, 'Y:', y(0),y(ny-1)
   ddely=(y(ny-2)-y(1))/(ny-3)  & sizet=size(xt) & nfl=sizet(1)
   yt=yt+ddely*float(nsh) 
   for i=0,nfl-1 do if (yt(i) lt y(1)) then yt(i)=yt(i)+y(ny-2)-y(1)
   for i=0,nfl-1 do if (yt(i) gt y(ny-2)) then yt(i)=yt(i)-y(ny-2)+y(1)
...
p1:  
    tit='Velocity, Field Lines and B_z' & tit2='B_N'    
    if contonly eq 'y' then begin
     ctfvec, a,vx,vy,pos1,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,smo,aval,xt,yt
     ctsca2, bx,pos3,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
    end else begin
    !P.MULTI=[0,5,0,0,0]
    vcfplot, bz,a,vx,vy,pos1,pos2,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,$
             smo,aval,xt,yt
    scplot2, bx,pos3,pos4,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
   endelse
  goto, menu
	
p2:
    tit=' Velocity, Field Lines and Vz' & tit2='Total pressure' 
    if contonly eq 'y' then begin
     ctvec, a,vx,vy,pos1,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,smo,aval
     ctsca2, p+bsq,pos3,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
    end else begin
    !P.MULTI=[0,5,0,0,0]
    vcfplot, vz,a,vx,vy,pos1,pos2,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,$
             smo,aval,xt,yt
    scplot2, p+bsq,pos3,pos4,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
   endelse
  goto, menu
...
p3:
   ...
   if contonly eq 'y' then begin
    ...
    vcfplot, rho,a,vx,vy,pos1,pos2,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,$
             smo,aval,xt,yt
...
p6:
   tit='Electric Field' & tit2='Current Density, J_z'
   if contonly eq 'y' then begin
     ctvec, ez,ex,ey,pos1,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,smo,aval
     ctsca2, jz,pos3,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
   end else begin
    vcplot, ez,a,ex,ey,pos1,pos2,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,smo,aval
    scplot2, jz,pos3,pos4,xpos,ypos,nl1,nl2,names,tit2,xtit,ytit1,smo
   endelse
  goto, menu

p7:
   tit='Magn Field, B_n' & tit2='Velocity, V_n'
   if contonly eq 'y' then begin
