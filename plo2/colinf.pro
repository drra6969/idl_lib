...
;--------------------
   ...
   xtit='-y (N)' & ytit='x(-M)' & smo='n'   & nsh=0  & nshtot=nsh  &  galpar='a'
...
;   ypmin=ymax  &  ypmax=ymin
   ypmin=ymin  &  ypmax=ymax
   print, 'after testbd',xpmin,xpmax
   print, 'after testbd',ypmin,ypmax
...
asymval, nx,ny,vzpro,vz1,vz2,vzav  
 print, 'vz_msp_orig = ', vz1,'  vz_msh_orig = ',vz2,'   vzav = ',vzav
     ...
     da=a(*,ny-1)-a(*,0)  & ast=total(da)/float(nx)  
     f1=a(*,0:ny-2)   & a(*,0:ny-2)=shift(f1,0,nsh)   
     if (nsh gt 0) then begin 
       a(*,ny-1)=a(*,0)
       a(*,0:(nsh-1))=a(*,0:(nsh-1))-ast
     endif
     if (nsh lt 0) then begin
       a(*,ny-1)=a(*,0)
       a(*,(ny-1+nsh):(ny-1))=a(*,(ny-1+nsh):(ny-1))+ast
     endif
...
;        device,/times,/bold,font_index=3
  ...
  if contin eq 'c' then begin
     xloadct,file='/users/ao/idl_lib/colortab.priv'
     coltab='c'
     goto, menu
  endif
...
; COORDINATES FOR PLOTS
   delx=xpmax-xpmin & dely=ypmax-ypmin & srat=(ymax-ymin)/(xmax-xmin)
...
p1:  
    tit=' Plasma Velocity' & tit2='Density'    
    if contonly eq 'y' then begin
...
p2:
    tit=' Magnetic Field' & tit2='Thermal Pressure' 
    if contonly eq 'y' then begin
    ...
    !P.MULTI=[0,5,0,0,0]
    vcfplot, bz,a,bx,by,pos1,pos2,xpos,ypos,nl1,nl2,names,tit,xtit0,ytit,$
             smo,aval,xt,yt
...
p4:
   tit='Total Pressure' & tit2='Plasma Beta'
   if contonly eq 'y' then begin
...
p7:
   tit='Magn Field, BN' & tit2='Velocity, VN'
   if contonly eq 'y' then begin
