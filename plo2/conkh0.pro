...
; CURRENT DENSITY J_Z AND ELECTRIC FIELD E_Z
  ...
  pi = 3.14159265536 & phi=80.0
  phir = phi*pi/180.0
  f2 = by*cos(phir) - bz*sin(phir) & f3 = bz*cos(phir) + by*sin(phir)
;  f2 = by*cos(phir) + bz*sin(phir) & f3 = -bz*cos(phir) + by*sin(phir)
;  by=f2 & bz=f3
;  f2 = vy*cos(phir) + vz*sin(phir) & f3 = -vz*cos(phir) + vy*sin(phir)
;  vy=f2 & vz=f3
...
;       !P.REGION=[0.,0.,1.0,1.25]
	...
	bmax=max(fa) & bmin=min(fa) & del=(bmax-bmin)/16.
        ...
        lvec=findgen(17)*del+bmin & lvec(16)=bav-0.3*del
        lvec=lvec(sort(lvec))
        print, 'lvecs:',lvec

	!P.POSITION=[xa1,ylo,xe1,yup]
	contour,fa,xf,yf,levels=lvec,$
        c_linestyle=lvec lt bav,$ 
...
;        title=' !9 r !X',xstyle=1,ystyle=1,$
        title=' SQRT(Bx**2+Bz**2)',xstyle=1,ystyle=1,$
...
;        bsq=by^2 
;        fa=interpolate(bsq,ioxf,ioyf,/grid) & fa=smooth(fa,3)
        fa=interpolate(by,ioxf,ioyf,/grid) & fa=smooth(fa,3)
	bmax=max(fa)
	bmin=min(fa)
        bav=0.5*(bmax+bmin)
        del=(bmax-bmin)/15.
	!P.POSITION=[xa2-hop,ylo,xe2-hop,yup]
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        c_linestyle=findgen(16)*del+bmin lt 0.4,$
...
; plot 5. page
	...
	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)') 
        ...
        f1=smooth(bz^2,3) & fa=interpolate(f1,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
        bav=0.5*(bmax+bmin)
        del=(bmax-bmin)/15.
