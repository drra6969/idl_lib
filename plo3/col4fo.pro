...
; GRID FOR CONTOUR AND SURFACE PLOTS
  ...
  colps ='n' & grayplot ='n' 
  plane='z' & whatcut='z' & choice='f' & igrid=1
  jnew= 'y' & enew= 'y'
...
;-----------PARAMETER----------------
  xmin = -10. & ymin = -20.  & zmin = -30.
  xmax =  10. & ymax =  20.   & zmax = 30.0
...
;
  if (fgroup eq '9') or (fgroup eq '11') then begin
    ...
    for k=1,nz-2 do fs3(*,*,k)=-0.5*dz(k)*fs3(*,*,k)
    if fgroup eq '11' then begin
      ftx=ftx+f1
      fty=fty+f2
      ftz=ftz+f3
...
print,'orient:',orient
  wxf=1. & wyf=1.  & xyrat=1.25
   ...
   ypos(1)=ymin+0.4*dely/dpy   ; next line
   ypos(2)=ymin+0.35*dely/dpy   ; location 'CASE'
...
;     pos4a=[xcb2+hop,ylo,xcb2+hop,yup]
   ...
   ypos=findgen(10)
   ypos(0)=ymin+0.2*dely            ; location for 'time'
   ypos(1)=ymin+0.15*dely/dpy   ; next line
   ypos(2)=ymin+0.125*dely/dpy   ; location 'CASE'
   ypos(3)=ymin+.85*dely             ; location 'Max='
   ypos(4)=ypos(3)-0.025*dely/dpy   ; next line
   ypos(5)=ymin+.65*dely    ; location 'Min='
   ypos(6)=ypos(5)-0.025*dely/dpy   ; next line
