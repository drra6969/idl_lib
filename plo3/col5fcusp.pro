...
;-----------PARAMETER----------------
  xmin = -8. & ymin = -8.  & zmin = -3.
  xmax = 8. & ymax =  8.   & zmax =  3.
...
menu:
  ...
  if choice eq 'p' then begin
      ...
      if contonly eq 'n' then begin 
        ...
        !p.color=0
        strgrid=string(igrid,'(i3.3)')
        if intplot eq 't' then strgrid='int'
        if colps eq 'n' then $
...
if choice eq 'f' then begin
  ...
  print, '        13 - 1/BB and beta'
  print, '        14 - potential phi and error'
  print, '         q - terminate'
  print, '    return - no changes applied'
...
;
  ...
  if fgroup eq '13' then begin
    ...
    for k=1,nz-2 do f3(*,*,k)=dz(k)*f3(*,*,k)
    imt='beta'
    f1=1./sqrt(bx*bx+by*by+bz*bz)
    ilim=where(f1 gt 10.) & f1(ilim)=10.
    f2=p*f1*f1
  endif

  if fgroup eq '14' then begin
    head1='Phi'
    head2='Error Phi'
    f1=vavg
    f2=verr
...
if intplot eq 't' then begin
  ...
  if plane eq 'z' then begin 
    ...
    xpmin=xmin  & xpmax=xmax  & ypmin=ymin  & ypmax=ymax 
    iz=26
    fc1=0.5/dz(iz)*f1(*,*,iz) & for k=iz+1,nz-iz do fc1=fc1+0.5/dz(k)*f1(*,*,k)
    fc2=0.5/dz(iz)*f2(*,*,iz) & for k=iz+1,nz-iz do fc2=fc2+0.5/dz(k)*f2(*,*,k)
    fc3=0.5/dz(iz)*f3(*,*,iz) & for k=iz+1,nz-iz do fc3=fc3+0.5/dz(k)*f3(*,*,k)
    fc1=reform(fc1) & fc2=reform(fc2) & fc3=reform(fc3) 
    if smo eq 'y' then begin
      fc1=smooth(fc1,3) & fc2=smooth(fc2,3) & fc3=smooth(fc3,3) & endif
    xplot=x  &  yplot=y
...
;   PLOTA
  ...
  if (fgroup eq '1') or (fgroup eq '6') or (fgroup eq '7') $
      or (fgroup eq '12') or (fgroup eq '13') or (fgroup eq '14') then begin 
     fp1=fa & fp2=fb 
  endif else begin
