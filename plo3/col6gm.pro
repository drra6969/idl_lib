...
;-----------PARAMETER----------------
  xmin =-1.5 & ymin = -100  & zmin = -1.5
  xmax = 1.5 & ymax =  100  & zmax =  1.5

; READ INPUT DATA OF DIMENSION NX, NY,NZ
  read3dh, dx,dxh,dy,dyh,dz,dzh, bx,by,bz,sx,sy,sz,rho,u,res,jx,jy,jz
...
menu:
  ...
  if choice eq 'c' then begin
     xloadct,file='/home/Xuan/idl_lib/colortab.priv';, block=1
     coltab='c'
     goto, menu
  endif
...
;   PLOTA
   ...
   grayplot=withps & if colps eq 'y' then grayplot='n'
    if coltab eq 'o' then $
     if withps ne 'y' then loadct,file='/home/Xuan/idl_lib/colortab.priv',ctdef $
      else $
      if colps eq 'y' then loadct,file='/home/Xuan/idl_lib/colortab.priv',ctps $
          else loadct,file='/home/Xuan/idl_lib/colortab.priv', 51
