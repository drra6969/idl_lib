...
; This version differs from Version 2 only in that the location of 
; the colortab file has been updated.
...
; GRID FOR CONTOUR AND SURFACE PLOTS
  ...
  colps ='y' & grayplot ='n' 
  plane='x' & whatcut='x' & choice='f' & igrid=2
  jnew= 'y' & enew= 'y'
  names=strarr(15) &  names=replicate(' ',15)
  ctab=findgen(nxf) &   cbary=fltarr(2,nxf)
   contonly = 'n' & orient='l'
  wlong=900 & wshort=720 & wsize=[700,900]
;   wlong=700 & wshort=700 & 
  wfact=1.0 &  wxf=1.0 & wyf=1.0 & psfact=0.9
;   wsize=[700,700] & 
wsold=[0,0] & wino=0 & srat0=1.1

;-----------PARAMETER----------------
; xmin = -20.0 & ymin =   -12.0  & zmin =   0.0
; xmax =   0.0 & ymax =    12.0  & zmax =  10.0
  xmin =  -12. & ymin =    -12.0  & zmin =  0.0
  xmax =   -1. & ymax =   12.0  & zmax =  3.0
...
grid3d, x,y,z,xmin,xmax,ymin,ymax,zmin,zmax,$
  ...
  ftx=f1 & fty=f2 & ftz=f3
    ...
    cutata='z=' & cutatb=string(x(igrid),'(f5.1)')
...
;     xloadct,file='/why2/fh/programs/IDL/IDL_lib/colortab.priv', block=1
     coltab='c'
     goto, menu
  endif
...
;           print,r,g,b
          ...
          t[!D.Table_Size-2] = 0
           write_png,'im'+imt+newplot+'_t'+string(time,'(i3.3)')+'_'+plane+ $
                      string(igrid,'(i3.3)')+'.png',imag, r,g,b,transparent=t
    endif
...
print,'orient:',orient
  wxf=1. & wyf=1.  & xyrat=1.25
   if orient eq 'l' then begin
     ...
     print, srat, dpx, dpy
     xinter=0.09 & cbthick=0.1*dpx & cbdist1=0.045 & cbdist2=0.055
     if withps eq 'n' then begin
...
;      window,wino,xsize=wsize(0),ysize=1.2*wsize(0),title=run 
     wsold = wsize
   endif
...
;   PLOTA
  ...
  if (fgroup eq '1') or (fgroup eq '6') or (fgroup eq '7') or (fgroup eq '12') then begin 
     fp1=fa & fp2=fb 
  endif else begin
