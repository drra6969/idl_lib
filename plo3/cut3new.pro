...
menu:
  ...
  if choice eq 'g' then begin
    if (withps eq 'n') then begin
           tvlct,r,g,b,/get
           imag=tvrd()
           write_gif, 'conp'+newplot+string(time,'(i3.3)')+'.gif',imag, r,g,b
    endif
...
;   PLOTA
    ...
    if wsmooth eq 's' then fp1=smooth(fa,3) else fp1=fa 
...
;    fptr=0.8
   ...
   if fpmax ne fpmin then begin
    ...
    if wsmooth eq 's' then fp2=smooth(fb,3) else fp2=fb 
...
;    fptr=1.9
   ...
   if fpmax ne fpmin then begin
    ...
    if wsmooth eq 's' then fp3=smooth(fc,3) else fp3=fc 
