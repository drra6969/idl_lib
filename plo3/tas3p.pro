...
DUMMY = 0.1 & DUMMY7 = FLTARR(7) & DUMMY15 = FLTARR(15)
...
WHICHFILE = 'C'
AFILE = ' '
BFILE = '/data2/as/mhd3/test/kurz/eta1/MAGTAP3'
  print, 'Input filenumber'
  read, fnumber

CFILE = 'PAGTAP'+string(fnumber,'(i1)')

;PRINT, 'WELCHE ART VON DATENFILE SOLL EINGELESEN WERDEN?'
;PRINT, '2D-AS(A), 3D-AS(B), 3D-GB(C)'
;READ, WHICHFILE
CASE WHICHFILE OF  'A': STOP
                        
                   'B': BEGIN
                        OPENR, 14, BFILE, /F77_UNFORMATTED
                        READU, 14, NX,NY,NZ,ANZPOS,ANZROW
                        END
                   'C': BEGIN
...
SX = FLTARR (NX,NY,NZ, /NOZERO)
SY = FLTARR (NX,NY,NZ, /NOZERO)
SZ = FLTARR (NX,NY,NZ, /NOZERO)

RHO =  FLTARR (NX,NY,NZ, /NOZERO)
U =    FLTARR (NX,NY,NZ, /NOZERO)
...
CASE WHICHFILE OF  'A': BEGIN
                        STOP
                        END
                   'B': BEGIN
READU, 14, ISCHRITT,IAUS,ISAFE,IEND,ITART,IVISRHO,NGLATT,$
           NAUS,IFEHLER,IGRID,IDIAG,FEHLER,DT,ZEIT,GAMMA,ETA,$            
           DUMMY7,RHO0,DUMMY15,$
           PERIO,LSYM,$
           SUPAK,$
           SUPAA,$
           XMIN,XMAX,YMIN,YMAX,ZMIN,ZMAX,AEQUI,ZENTR,EPS,$
           XVEC,DIFX,DDIFX,DDIFPX,DDIFMX,MEANPX,MEANMX,$
...
READU, 14, BX,BY,BZ
READU, 14, SX,SY,SZ
READU, 14, RHO,U,RES,PROF
READU, 14, ISCHRITT,IAUS,ISAFE,IEND,DT,GAMMA,ETA,GRAV,RHO0
...
PRINT, ''
PRINT, 'BFELD(BX,BY v BZ), JFELD(JX,JY v JZ), GESCHW(VX,VY v VZ)?'
PRINT, 'EFELD(EX,EY v EZ), DICHTE(D), DRUCK(P), RESISTIVITAET(R)?' 
PRINT, 'JPARALLEL(JP), EPARALLEL(EP), GRADIENTP(PGX,PGY,PGZ)?'
PRINT, 'ODER LIEBER PFEILCHENMALEN(PF)
READ, PLOWAHL
CASE WHICHFILE OF 'B': TEILER = 4.                
                  'C': TEILER = 1.
ENDCASE
...
DIFX   = DIFX/TEILER
DDIFX  = DDIFX/TEILER
DDIFPX = DDIFPX/TEILER
DDIFMX = DDIFMX/TEILER
MEANPX = MEANPX/TEILER
MEANMX = MEANMX/TEILER
DIFY   = DIFY/TEILER
DDIFY  = DDIFY/TEILER
DDIFPY = DDIFPY/TEILER
DDIFMY = DDIFMY/TEILER
MEANPY = MEANPY/TEILER
MEANMY = MEANMY/TEILER
DIFZ   = DIFZ/TEILER
DDIFZ  = DDIFZ/TEILER
DDIFPZ = DDIFPZ/TEILER
DDIFMZ = DDIFMZ/TEILER
MEANPZ = MEANPZ/TEILER
MEANMZ = MEANMZ/TEILER
RHO    = RHO/TEILER
U      = U/TEILER
RES    = RES/TEILER
BX     = BX/TEILER
...
CASE PLOWAHL OF 'D':  g_feld = RHO
      ...
      FOR  IZ = 0,NZ1 DO BEGIN
       ...
       FOR IY = 0,NY1 DO BEGIN
        FOR IX = 0,NX1 DO BEGIN
        FELDY(IX,IY,IZ) = 2.*U(IX,IY,IZ)^GAMMA
                         ENDFOR
                         ENDFOR
                         ENDFOR
      FOR  IZ = 0,NZ1 DO BEGIN
       ...
       FOR IY = 0,NY1 DO BEGIN
        FOR IX = 0,NX1 DO BEGIN
        FELDZ(IX,IY,IZ) = 2.*U(IX,IY,IZ)^GAMMA
                         ENDFOR
                         ENDFOR
                         ENDFOR
  ...
  FOR IZ = 0,NZ-1 DO BEGIN
   ...
   FOR IZ = 0,NZ-1 DO BEGIN
	 ...
	 FELDX(IX,IY,IZ) = DIFY(IY*NX+1)*(BZ(IX,IY+1,IZ)-BZ(IX,IY-1,IZ))$
                         - DIFZ(IZ)*(BY(IX,IY,IZ+1)-BY(IX,IY,IZ-1))
         HELP(IX,IY,IZ) = RES(IX,IY,IZ)*FELDX(IX,IY,IZ)$
...
print, ' '
print, ' '
if (c_plot eq 5) then goto, jump40
...
zrw = 'm'
if (c_plot eq 3) or (c_plot eq 4) then begin
...
READU, 14, ISCHRITT,IAUS,ISAFE,IEND,ITART,IVISRHO,NGLATT,$
           ...
           ISAT,DUMMY7
                        END
                   'C': BEGIN
READU, 14, ISCHRITT,IAUS,ISAFE,IEND,DT,ZEIT,GAMMA,ETA,RHO0
READU, 14, XMIN,XMAX,YMIN,YMAX,ZMIN,ZMAX           
READU, 14, XVEC,DIFX,DDIFX,DDIFPX,DDIFMX,MEANPX,MEANMX,$
           YVEC,DIFY,DDIFY,DDIFPY,DDIFMY,MEANPY,MEANMY,$
           ZVEC,DIFZ,DDIFZ,DDIFPZ,DDIFMZ,MEANPZ,MEANMZ
...
;        FOR IX = 1,NX-2 DO BEGIN
;          IV = IX + (IY-1)*NX
;          HELP(IX,IY,IZ) =  (1.-0.75)*U(IX,IY,IZ)$
;                        + 0.25*(MEANPX(IV)*U(IX+1,IY,IZ)$
;                              + MEANMX(IV)*U(IX-1,IY,IZ)$
;                              + MEANPY(IV)*U(IX,IY+1,IZ)$
;                              + MEANMY(IV)*U(IX,IY-1,IZ)$
...
;        FOR IX = 1,NX-2 DO BEGIN
;          IV = IX + (IY-1)*NX
;          FELDX(IX,IY,IZ) =  (1.-0.75)*SX(IX,IY,IZ)$
;                        + 0.25*(MEANPX(IV)*SX(IX+1,IY,IZ)$
;                             + MEANMX(IV)*SX(IX-1,IY,IZ)$
;                              + MEANPY(IV)*SX(IX,IY+1,IZ)$
;                              + MEANMY(IV)*SX(IX,IY-1,IZ)$
;                              + MEANPZ(IZ)*SX(IX,IY,IZ+1)$
;                              + MEANMZ(IZ)*SX(IX,IY,IZ-1) )
;          FELDY(IX,IY,IZ) =  (1.-0.75)*SY(IX,IY,IZ)$
;                        + 0.25*(MEANPX(IV)*SY(IX+1,IY,IZ)$
;                              + MEANMX(IV)*SY(IX-1,IY,IZ)$
;                              + MEANPY(IV)*SY(IX,IY+1,IZ)$
;                              + MEANMY(IV)*SY(IX,IY-1,IZ)$
;                              + MEANPZ(IZ)*SY(IX,IY,IZ+1)$
;                              + MEANMZ(IZ)*SY(IX,IY,IZ-1) )
;          FELDZ(IX,IY,IZ) =  (1.-0.75)*SZ(IX,IY,IZ)$
;                        + 0.25*(MEANPX(IV)*SZ(IX+1,IY,IZ)$
;                              + MEANMX(IV)*SZ(IX-1,IY,IZ)$
;                              + MEANPY(IV)*SZ(IX,IY+1,IZ)$
;                              + MEANMY(IV)*SZ(IX,IY-1,IZ)$
