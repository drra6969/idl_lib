...
Pro infield
  ...
  COMMON field, ff, xmin,xmax,ymin,ymax,zmin,zmax,noset,datnum,tnum,time
  mx=long(131) & my=long(126) & mz=long(126) 
  time=0.0 
  
  
  for i=1, noset do begin
    name='mflux'+string(datnum(i),'(i1.1)')+string(tnum,'(i3.3)')
    ...
    readu, 8,  x,y,z
    nuno0=long(131) & nuall=long(131)
    readu, 8, nuno0,nuall
    print, nuno0,nuall
    ifff=lonarr(nuall,/NOZERO) & wff=fltarr(nuall,/NOZERO)  
...
;  idir = axis, 0 for x, 1 for Y, 2 for Z. slice = plane number.
  COMMON field, a, xmin,xmax,ymin,ymax,zmin,zmax,noset,datnum,tnum,time
  COMMON heini,dims,amin,amax,axex,axrev,p0cube,p0,p1,pb, $
         v_close,vfaces,facevs,z_last,zb_last,mode, $
         idir,islice,iaxes,ax,az,zscale,persp,isop, $
         resolution, pmenu
...
;  print,'in isosurf'
  ...
  if n_elements(verts) eq 0 then begin
    ...
    pz = convert_coord([-d1(0)/16,-d1(1)/16,d1(2)/3], $
          /T3D, /TO_DEV, /DATA)
    if ((pmenu eq 'a') and (az gt -5.) and (az lt 185.)) then $
    xyouts, pz(0),pz(1),'Z',charsize=3.0,charthick=3,/device
  endelse
end
...
;  azf((nf1+nf2):(nf1+nf2+nf3-1))=az2+dz3*findgen(nf3)+dz3
  ...
  axf(*) = ax0+20.0*( sin( pi*ei(*)/float(nftot-1) ) )
  azf(*) = az0-180.0/(nftot-1)*ei
  perspf(*)= persp
;  axf(*) = ax
;  azf(*) = az
;  perspf(*)= persp
...
;     draw_slice
     imgif = tvrd()
     gifname='flux'+string((ip+35),'(i2.2)')+'.gif'
...
;    device,/color,bits_per_pixel=8,filename='p.eps',$
...
;  print, 'p facevs eq v_close',p
  ...
  xoo=resolution(0) & yoo=resolution(1) 
  xyouts,0.15*xoo,0.95*yoo,'Evolution of Magnetic Flux Tubes',$
      charsize=2.5,charthick=3,/device
  xyouts,0.7*xoo,0.89*yoo,'time:'+string(time,'(f5.1)'),$
...
isop = { ISOP, hi_lo : 1, value: 50.0, window : 0, drawable : 0L, $
  ...
  noset=1 & tnum=1 & dumm=1 
  datnum=intarr(6)
  print, 'number of datasets(<= 3)?'
  read, noset
  print, 'time index (tnum: up to 3 digits!))?'
  read, tnum
  
  for i=1, noset do begin
    print, 'datasetindex (datnum: <= 3)?'
    read, dumm
    datnum(i) = dumm
  endfor
  infield
...
; vertex numbers vs Edge index (12 edges)
   ...
   orthop = [ 0., 0., 1., -dims(2)/2.]	;Initial orthogonal plane
    ...
    ax=20.0 & az=55.0 & zscale=1.4 & persp=1.4 & ino=0.0
