...
Pro infield
  ...
  COMMON field, fslice, xmin,xmax,ymin,ymax,zmin,zmax,fnumber,time
...
;    COMMON VOLUME_DATA, fslice, xmin,xmax,ymin,ymax,zmin,zmax,fnumber,time
...
;  idir = axis, 0 for x, 1 for Y, 2 for Z. slice = plane number.
  COMMON field, a, xmin,xmax,ymin,ymax,zmin,zmax,fnumber,time
  COMMON heini,dims,amin,amax,axex,axrev,p0cube,p0,p1,pb, $
         v_close,vfaces,facevs,z_last,zb_last,mode, $
         idir,islice,iaxes,ax,az,zscale,persp,isop, $
         resolution, pmenu
...
;     draw_slice
     imgif = tvrd()
     gifname='gifout'+string((ip+45),'(i2.2)')
...
;  print, 'p facevs eq v_close',p
  ...
  xyouts,0.15*xoo,0.95*yoo,'Evolution of the Current Layer',$
      charsize=2.5,charthick=3,/device
  xyouts,0.7*xoo,0.89*yoo,'time:'+string(time,'(f5.1)'),$
      charsize=2.0,charthick=3,/device
...
; vertex numbers vs Edge index (12 edges)
   ...
   orthop = [ 0., 0., 1., -dims(2)/2.]	;Initial orthogonal plane
    ...
    ax=20.0 & az=250.0 & zscale=1.4 & persp=1.4 & ino=0.0
    pslice='n' & kslice='n' & idir=0 & islice=dims(idir)/2
    pcolor='n' & piso='n' & isoin=58.0 & isolevel=0.5
;    isop.value=isolevel/100.0*(amax-amin)+amin & isop.hi_lo=1
    isop.value=isolevel & isop.hi_lo=1
    panim='n'
