; START OF MAIN PROGRAM
  ...
  nxf=81 & nyf=81 & nzf=81     ;grid for uniform interpol fields
...
;----------INPUT--------------------
  ...
  openr, 8, name,/F77_UNforMATTED
  readu, 8, time, nx, ny, nz, x,y,z, rho
;  readu, 8,  x,y,z, rho
; print, x,y,z
...
gridindex:
  ...
  if plane eq 'a' then begin
    ...
    VT3D, 'x', x(igrid1), xn,yn,zn, vecx1,vecy1,xar1,yar1
    VT3D, 'y', y(igrid2), xn,yn,zn, vecx2,vecy2,xar2,yar2
    VT3D, 'z', z(igrid3), xn,yn,zn, vecx3,vecy3,xar3,yar3
  endif
  if plane eq 'x' then begin
    ...
    fc2=f1(igrid2,*,*) & fc2=reform(fc2) 
    fc3=f1(igrid3,*,*) & fc3=reform(fc3) 
    VT3D, 'x', x(igrid1), xn,yn,zn, vecx1,vecy1,xar1,yar1
    VT3D, 'x', x(igrid2), xn,yn,zn, vecx2,vecy2,xar2,yar2
    VT3D, 'x', x(igrid3), xn,yn,zn, vecx3,vecy3,xar3,yar3
  endif
  if plane eq 'y' then begin
    ...
    VT3D, 'y', y(igrid1), xn,yn,zn, vecx1,vecy1,xar1,yar1
    VT3D, 'y', y(igrid2), xn,yn,zn, vecx2,vecy2,xar2,yar2
    VT3D, 'y', y(igrid3), xn,yn,zn, vecx3,vecy3,xar3,yar3
  endif
  if plane eq 'z' then begin
    ...
    fc2=f1(*,*,igrid2) & fc2=reform(fc2) & fc2=rotate(fc2,4)
    fc3=f1(*,*,igrid3) & fc3=reform(fc3) & fc3=rotate(fc3,4)
    VT3D, 'z', z(igrid1), xn,yn,zn, vecx1,vecy1,xar1,yar1
    VT3D, 'z', z(igrid2), xn,yn,zn, vecx2,vecy2,xar2,yar2
    VT3D, 'z', z(igrid3), xn,yn,zn, vecx3,vecy3,xar3,yar3
  endif
...
;    if (fpmax*fpmin lt 0.0) then fptr=0.0 else fptr=0.5*(fpmax+fpmin)
    ...
    xyouts,xpos,yposcut,cutata+cutatb2,font=2
    xyouts,xpos,yposmin,'Min/Max ='+string(fpmin,'(f5.2)')$
...
;    if (fpmax*fpmin lt 0.0) then fptr=0.0 else fptr=0.5*(fpmax+fpmin)
    ...
    xyouts,xposyt,yposyt,ytit,font=2
    xyouts,xpos,ypost,'t ='+string(time,'(i3)'),font=2
    xyouts,xpos,yposcut,cutata+cutatb3,font=2
    xyouts,xpos,yposmin,'Min/Max ='+string(fpmin,'(f5.2)')$
                        +'/'+string(fpmax,'(f5.2)'),font=2
    xyouts,xpos,yposrun,run
...
;----------Third Page--------------------
  ...
  if (contin eq '' or contin eq 'y') then begin
    ...
    fpmax=sqrt(max(vecx1^2+vecy1^2))
    !P.POSITION=pov1
    ...
    fpmax=sqrt(max(vecx2^2+vecy2^2))
    !P.POSITION=pov2
    ...
    fpmax=sqrt(max(vecx3^2+vecy3^2))
    !P.POSITION=pov3
