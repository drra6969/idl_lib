...
PRO orient
  COMMON heini,dims,amin,amax,axex,axrev,p0cube,p0,p1,pb, $
         v_close,vfaces,facevs,z_last,zb_last,mode,orig, $
...
PRO draw_coor, c0, c1, faces, win
;	draw coordinate system from origin: orig
;	and [c1(0), c1(1), p2(3)].
;	color = drawing color.
  COMMON heini,dims,amin,amax,axex,axrev,p0cube,p0,p1,pb, $
         v_close,vfaces,facevs,z_last,zb_last,mode,orig, $
...
;  print,'in draw_coor'
  ...
  !Z.THICK=2.5
  d1 = dims-1
  ppx=fltarr(3,2) & pp1 = ppx
  ppy=fltarr(3,2) & pp2 = ppy
  ppz=fltarr(3,2) & pp3 = ppz
  ppx(*,0)=orig & ppx(*,1)=orig & ppx(0,1)=orig(0)+0.25*d1(0)
  ppy(*,0)=orig & ppy(*,1)=orig & ppy(1,1)=orig(1)+0.25*d1(1)
  ppz(*,0)=orig & ppz(*,1)=orig & ppz(2,1)=orig(2)+0.25*d1(2)
  pp1 = convert_coord(ppx, /T3D,/TO_DEVICE,/DATA)
  pp2 = convert_coord(ppy, /T3D,/TO_DEVICE,/DATA)
  pp3 = convert_coord(ppz, /T3D,/TO_DEVICE,/DATA)
  plots, pp1, /dev
  plots, pp2, /dev
  plots, pp3, /dev
    px = convert_coord([orig(0)+0.2*d1(0),orig(1)-d1(1)/16, $
           orig(2)-d1(2)/16], /T3D, /TO_DEV, /DATA)
    py = convert_coord([orig(0)-d1(0)/16,orig(1)+0.28*d1(1), $
           orig(2)], /T3D, /TO_DEV, /DATA)
    pz = convert_coord([orig(0)-d1(0)/20,orig(1)-d1(1)/16, $
           orig(2)+0.2*d1(2)], /T3D, /TO_DEV, /DATA)
...
;    device,xsize=8,ysize=8,xoffset=2,yoffset=2
    ...
    ind=where(image ne 0B)
    image(where(image eq 255B)) = min(image(ind))
    print, max(image), min(image)
    image(where(image eq 0B))=max(image)
    print, max(image), min(image)
    tvscl, image
    device,/close
    device,filename='p1.ps'
    image=255B-image
    tvscl, image
    device,/close

    device,filename='p2.ps'
    print, 'ineg max, min ', max(image), min(image)
    ind=where(ineg ne 0B)
    ineg(where(ineg eq 0B)) = min(ineg(ind))
    print, 'ineg max, min ', max(image), min(image)
    ind=where(ineg ne 255B)
    ineg(where(ineg eq 255B)) = max(ineg(ind))
    print, 'ineg max, min ', max(image), min(image)
    tvscl, ineg
    device,/close
...
;    resolution(1) = 6 * resolution(0) / 7
    ...
    rsize=[xmax-xmin,ymax-ymin,zmax-zmin]
    rorig=[5.0-xmin,0.0-ymin,0.0-zmin]
    orig=rorig/rsize*d1
...
; vertex numbers vs Edge index (12 edges)
   ...
   orthop = [ 0., 0., 1., -dims(2)/2.]	;Initial orthogonal plane
    ...
    ax=20.0 & az=-35.0 & zscale=1.4 & persp=1.0 & ino=0.0
    pslice='n' & kslice='n' & idir=0 & islice=dims(idir)/2
    pcolor='n' & piso='n' & isoin=58.0 & isolevel=20.0
