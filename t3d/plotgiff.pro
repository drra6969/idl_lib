PRO plotgif
  ...
  for ip=0,nftot-1 do begin
     ...
     READ_GIF, gifname, imgif
     tv, imgif
  endfor

  nf1=33 & nf2=32 & nf3=32 & 
  nftot=61          ;or  nftot=nf1+nf2+nf3 
...
;    device,/color
    ...
    device,/color,bits_per_pixel=8,filename='p.ps',$
           xsize=15.,ysize=15.,xoffset=1.5,yoffset=1.5
...
;    device,xsize=14,ysize=14,xoffset=0,yoffset=2
    ineg=255B-image
    image1=image
    print, 'max(image):', max(image), '  min(image):', min(image)
    ...
    tvscl, image1
;    device,/close
    device, xsize=10.,ysize=10.,xoffset=1.5,yoffset=10.
;    image=255B-image
    tv, ineg
    device,/close
...
; vertex numbers vs Edge index (12 edges)
   ...
   orthop = [ 0., 0., 1., -dims(2)/2.]	;Initial orthogonal plane
    ...
    ax=20.0 & az=75.0 & zscale=1.4 & persp=1.4 & ino=0.0
