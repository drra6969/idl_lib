; $Id: slown.pro,v 1.2 1993/06/25 23:16:28 dave Exp $
...
Function slown_plane_int, dummy
...
COMMON slown_common, dims, sl, z_last, zb_last, mode, fixed
...
a=atan(u(1,*)-u(1,dmin), u(0,*)-u(0,dmin))
...
PRO SLOWN_UNDO		;Undo last operation
COMMON slown_common, dims, sl, z_last, zb_last, mode, fixed
...
PRO slown_orientation, i	;i = orientation
...
ENDELSE
bad_aspect:
SLOWN_JOURNAL, "ORI", [sl.axex, sl.axrev, sl.rotation, f ]
...
PRO slown_oblique		;Do an oblique slice
...
SLOWN_JOURNAL, "TRANS", [sl.trans, sl.threshold * 100. / sl.nc3]
SLOWN_JOURNAL, "SLICE", [az, el, sl.interp, sl.expose, 1., z0 ]

if n_elements(d0) le 0 then return		;Anything?
...
PRO slown_draw, ax, slice		;draw a slice.
...
SLOWN_JOURNAL, "TRANS", [sl.trans, sl.threshold * 100. / sl.nc3]
SLOWN_JOURNAL, "SLICE", [ax, slice, sl.interp, sl.expose, 0.]
...
PRO slown_colors, table	;load our color table
...
SLOWN_JOURNAL, "COLOR", [table, sl.stretch, 100. * sl.shading]
...
WIDGET_CONTROL, sl.base, /HOURGLASS
SLOWN_JOURNAL, "ISO", [(sl.isop.value-sl.amin)/(sl.amax-sl.amin)*100, $
		sl.isop.hi_lo]
set_plot,'Z'
...
PRO slown_erase		;draw the background
...
if n_elements(d0) lt 1 then return
...
if sl.ortho eq 0 then begin		;Oblique???
	...
	if valid and (sl.lbuttons ne 0) THEN BEGIN  ;Update obliq outline
	    WIDGET_CONTROL, sl.pos_text, set_value = $
		STRING(z0, az, el, FORMAT="('(',3f5.1,') A=', i4, ' E=',i4)")
	    d0 = slown_plane_int()		;New intersections
...
ENDIF ELSE BEGIN			;Orthogonal.
	...
	IF (ev.release eq 1) and (old_slice ge 0) THEN BEGIN
	    device, SET_GRAPH=3
	    slown_draw, fixed, old_slice	;Mark the slice
	    ENDIF			;Release
...
SLOWN_JOURNAL, "TRANS", [sl.trans, sl.threshold * 100. / sl.nc3]
SLOWN_JOURNAL, "CUBE", [ mode, sl.cut_ovr, sl.interp, reform(sl.p0cube, 6) ]
...
print, ev.id, eventval
case eventval of
"CANCUBE" : mark_cube		;Undraw outline
"COLORS" : slown_colors, (where(sl.color_button eq ev.id))(0)
...
"GOCUBE" : do_cube
"HELP" : BEGIN
	xdisplayfile, filepath("slown_demo.txt", subdir='help'), $
		title = "slown demo help", $
		group = ev.top, $
		width = 72, height = 24
	return
	ENDCASE
...
"ORIENTATION": SLOWN_ORIENTATION, (where(sl.ori_butt eq ev.id))(0)
"Postscript" : begin
               slown_postscript
               print,'nu ps called'
               endcase
...
PRO slown, GROUP = group, RANGE = range, COMMANDS = commands, $
...
;			This command sets the orientation for the SLOWN 
...
;
;     CMD_FILE:	A string that contains the name of a file containing SLOWN
;		commands to execute as described above.
;
;	DETACHED: if set, put the drawable in a separate window. (Good
;		for large windows.)
;	GROUP:	The base ID of the widget that calls SLOWN.  When this 
...
;	COMMON SLOWN_COMMON   ;Used internally.
;
; SIDE EFFECTS:
...
;
; PROCEDURE:
;	The slown program has the following modes:
...
;
; EXAMPLE:
;	Data is transferred to the SLOWN via the VOLUME_DATA common block
...
;	in the SLOWN.  Before you read the data, establish the VOLUME_DATA
...
;		CLOSE, 1
;
;	Now you can run the SLOWN widget application by entering:
;
;		SLOWN
;
;	The data stored in VOL is the data being worked on by the SLOWN.
;
...
if XRegistered("slown") THEN RETURN
...
if keyword_set(detached) THEN BEGIN
	rbase = WIDGET_BASE(Title='Slown', EVENT_PRO='SLOWN_EVENT')
	sl.rbase = rbase
...
parent = sl.mode_bases(5)           ;Color tables
...
sl.file_text(0) = WIDGET_TEXT(junk, xsize=24, ysize=1, $
	value='slown.jou'+string(replicate(32b,14)), /EDIT, /FRAME)
...
slown_orientation,6		;Reset to default orientation, erase
slown_colors, 0

if n_elements(commands) gt 0 then slown_playback, commands  ;Execute cmds?
if n_elements(cmd_file) gt 0 then slown_playback, file = cmd_file
...
tbl=0				;Kill it
XManager, "slown", sl.base, EVENT_HANDLER = slown_events, GROUP = group
end
