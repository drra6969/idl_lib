PRO plt1eqs, min,max,timeb,timev,baxr,bayr,bazr,strnn
...
;        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
	...
	if max(4.*rhor) le max(babr) then $
           plot, 60*timev, 10.*rhor, $
           xtickformat='XTICKS', $
	   yrange=[0,bmax], xstyle=1,ystyle=9,xtickname=names $
        else plot, 60*timev, rhor, $
           xtickformat='XTICKS', $
	   yrange=[0,bmax], xstyle=1,ystyle=9,xtickname=names
	oplot, 60*timeb, babr, line=1
;	ytick_get=vv

        tempmax=max(tempr) & print, 'Temp, max: ',tempmax
        temprange = 10000.   & tempscale = bmax/temprange
	axis,yaxis=1,yrange=[0,temprange],ystyle=1
	oplot, 60*timev, tempscale*tempr, line=2
  ...
  xyouts, .45, 0.88, titl, charsize=1.55,charthick=2.0,/norm, alignment=0.5
	...
	!P.POSITION=[xa,ylo2,xe,yup2]
        ...
        delb=bmax
        bmax=bmax+0.1*delb  & delb=bmax
	plot, 60*timev, ptotr, yrange=[0,bmax], $
           xtickformat='XTICKS', $
	   xstyle=1,ystyle=1,xtickname=names
	oplot, 60*timev, pr, line=1
	oplot, 60*timev, pbr, line=2
	...
	plot, 60*timev, vxr, yrange=[bmin,bmax], $
           xtickformat='XTICKS', $
	   xstyle=1,ystyle=1,xtickname=names, line=2
	oplot, 60*timev, vyr, line=1
	oplot, 60*timev, vzr, line=0
        yt1=bmin+0.73*delb    
        yt1a=bmin+0.60*delb    
        yt2=bmin+0.45*delb    
        yt3=bmin+0.3*delb    
	...
	!P.POSITION=[xa,ylo4,xe,yup4]
        ...
        delb=bmax-bmin
        bmax=bmax+0.05*delb & bmin=bmin-0.05*delb & delb=bmax-bmin
	plot, 60*timeb, bxr, yrange=[bmin,bmax], $
           xtickformat='XTICKS', xminor=6, $
	   xstyle=1,ystyle=1,xtitle=xtit, line=2
	oplot, 60*timeb, byr, line=1
	oplot, 60*timeb, bzr, line=0
        yt1=bmin+0.73*delb   
        yt1a=bmin+0.60*delb   
        yt2=bmin+0.45*delb    
        yt3=bmin+0.3*delb    
