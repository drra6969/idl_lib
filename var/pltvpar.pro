PRO pltvpar, min,max,timeb,timev,baxr,bayr,bazr,strnn
...
;	ytick_get=vv
  ...
  xyouts, .45, 0.88, titl, charsize=1.55,charthick=2.0,/norm, alignment=0.5

	!P.POSITION=[xa,ylo2,xe,yup2]
	    vpar=(vxr*bxr+vyr*byr+vzr*bzr)/sqrt(bxr^2+byr^2+bzr^2)
        bmax=max(vpar) & print, 'vpar max: ',bmax
        bmin=min(vpar) & print, 'vpar min: ',bmin
        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
        delb=bmax-bmin
        bmin=bmin-0.1*delb  & delb=bmax
        bmax=bmax+0.1*delb  & delb=bmax
        
	plot, timev, vpar,$
	   yrange=[bmin,bmax], $
	   xstyle=1,ystyle=1,xtickname=names
        xt1=max+0.02*del   &  yt1=bmin+0.9*delb    
        xt1a=max+0.04*del  &  yt1a=bmin+0.60*delb    
        xt2=max+0.04*del   &  yt2=bmin+0.45*delb    
        yt3=bmin+0.3*delb    
        yt4=bmin+0.15*delb    
        xyouts, xt1, yt1,'V_parallel',charsize=1.5
        xyouts, xt1a, yt1a,'(km/s)',charsize=1.5
	...
	plot, timev, vxr, yrange=[bmin,bmax], $
	   xstyle=1,ystyle=1,xtickname=names, line=2
	oplot, timev, vyr, line=1
	oplot, timev, vzr, line=0
        yt1=bmin+0.73*delb    
        yt1a=bmin+0.60*delb    
        yt2=bmin+0.45*delb    
        yt3=bmin+0.3*delb    
	...
	!P.POSITION=[xa,ylo4,xe,yup4]
        ...
        delb=bmax-bmin
        bmax=bmax+0.05*delb & bmin=bmin-0.05*delb & delb=bmax-bmin
	plot, timeb, bxr, yrange=[bmin,bmax], $
	   xstyle=1,ystyle=1,xtitle=xtit, line=2
	oplot, timeb, byr, line=1
	oplot, timeb, bzr, line=0
        yt1=bmin+0.73*delb   
        yt1a=bmin+0.60*delb   
        yt2=bmin+0.45*delb    
        yt3=bmin+0.3*delb    
