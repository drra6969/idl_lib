...
;the line under is moved here from above 
  ...
  if ( (whatindex ne '') and  (whatindex ne 'i') $
     ...
     and (whatindex ne 'h') and (whatindex ne 'b') and (whatindex ne 'q'))    then begin
    index=fix(whatindex)
  ...
  if whatindex eq 'p' then begin
     ...
     !Y.THICK=1.5
     !P.CHARTHICK=2.
     psname='satp'+string(index,'(i3.3)')+'p'+string(countps,'(i2.2)')
...
plotall:
  ...
  plotvarsimh,tfac*tmin,tfac*tmax, strnn,coordstrn,base,eb1,eb2,eb3,vht,$
                withht,varpresent,beta0,tvarmin,tvarmax,smo
  if withps eq 'y' then  begin 
