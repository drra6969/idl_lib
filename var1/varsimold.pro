...
COMMON simdat3, nx,ny,nz,x,y,z,dx,dy,dz,bx,by,bz,vx,vy,vz,rho,u,res,p,$
                xmin,xmax,ymin,ymax,zmin,zmax,linealong,icut
...
;     boltz = 1.38*10^(-23)           ; 
    ...
    tempfac=tempnorm 
    tfac=tnorm & if (satchoice eq '2' or satchoice eq '3') then tfac=1.0 
    time2d=tnorm*time2d
...
; 1. Rotation into an approximate b
; No rotationoundary normal system (GSM)
;       i.e. x sunward y nortward.
  tr0=fltarr(3,3) & tr0(0,0)=1. & tr0(1,0)=0. & tr0(2,0)=0. 
                    tr0(0,1)=0. & tr0(1,1)=1.  & tr0(2,1)=0.
...
cut: 
  ...
  if whatindex eq 'o' then begin
    unitype='p'
    bfac=bnorm & vfac=vnorm & lfac=lnorm & nfac=nnorm & pfac=pnorm
    tempfac=tempnorm & vafact=vanorm & 
    if (satchoice ne '2' or satchoice ne '3') then tfac=tnorm  
    t=tfac*time
...
; Same for the boundary values
  ...
  if (satchoice eq '2' or satchoice eq '3') then  $
      strnn=cas+'Cut at z= '+string(zsi0(index),'(f5.1)')$
                 +', phi='+string(phi,'(f5.1)')$
...
; -------------------------------
  if (whatindex eq 'v') then begin
    ...
    if (satchoice eq '2' or satchoice eq '3') then  $
      strnn='!17'+cas+'Cut at z= '+string(zsi0(index),'(f5.1)')$
                 +', phi='+string(phi,'(f5.1)')$
                 +', time='+string(time2d,'(f5.0)')
    print,strnn
    tv=t(iv1:iv2)
