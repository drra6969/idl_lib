...
;          f25=v_units*vey(igrid,*) & f26=v_units*vez(igrid,*)
          ...
          f36=cnv(igrid,*)
          f37=hcd(igrid,*)
          f38=f31+f32+f33+f34+f35+f36+f37
...
;          f25=v_units*vey(*,igrid) & f26=v_units*vez(*,igrid)
  ...
  print, 'plot page, Terms in Electron Heating'
...
;-----------------------------------------------------------------------
  ...
  if (contin eq '' or contin eq 'y') then begin
	...
	!P.POSITION=[xe1,ylo,xa3,yup]
        bmax=max([f31,f32,f33,f34,f35,f36,f37,f38]) & print, 'Rho max: ',bmax
        bmin=min([f31,f32,f33,f34,f35,f36,f37,f38]) & print, 'Rho min: ',bmin
        bmax=0.1*bmax
        bmin=0.1*bmin
        delb=bmax-bmin
	...
	plot, smooth(f31,3),plcoo, $
	title='Electron Energy Budget (eV/s)', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
;	oplot, smooth(f32,3), plcoo, line=1
	oplot, smooth(f33,3), plcoo, line=2,thick=3
	oplot, smooth(f34,3), plcoo, line=3,thick=3
;	oplot, smooth(f35,3), plcoo, line=4
	oplot, smooth(f36,3), plcoo, line=5
	oplot, smooth(f37,3), plcoo, line=2,thick=3
	oplot, smooth(f38,3), plcoo, line=0,thick=3
...
save,file='rayheatdown.dat',/xdr,time,plcoo,f31,f32,f33,f34,f35,f36,f37,f38
