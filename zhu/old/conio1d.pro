...
;  print, y
   ...
   readu, 8,  x,dx,g2,g3,g3,g3,g3, y,dy,h2,h3,h3,h3,h3
   readu, 8,  vnx,vny,vnz
   readu, 8,  rhon,un,rhona,una,nu12s
   close, 8
...
;        device,/times,/bold,font_index=3
  ...
  print, 'plot first page? Rhon, Tn, flow vectors'
  read, contin
...
;       !P.REGION=[0.,0.,1.0,1.25]
	...
	contour,fa,xf,yf,levels=lvec,$
        ...
        title=' Nn-Nn0 (cm^(-3))',xstyle=1,ystyle=1,$
        xtitle='x (km)',ytitle=ytit
	xyouts,xpos,ypos0,'time'
	xyouts,xpos,ypos1,' '+string(time,'(f5.1)') 
	xyouts,xpos,ypos2,run
	xyouts,xpos,ypos3,'Max'
	xyouts,xpos,ypos4,' '+string(bmax*3*1e5,'(E9.1)')
	xyouts,xpos,ypos5,'Min'
	xyouts,xpos,ypos6,' '+string(bmin*3*1e5,'(E9.1)')

;        pn=2*un^(5/3)
;        pna=2*una^(5/3)
;        fa=interpolate(pn-pna,ioxf,ioyf,/grid) 
;	bmax=max(fa,indmax) & bmin=min(fa,indmin)
;        bav=0.5*(bmax+bmin)
...
;	contour,fa,xf,yf,levels=lvec,$
;        c_linestyle=lvec lt 0.0,$
;        title=' Pn (nPa)',xstyle=1,ystyle=1,$
;        xtitle='x',ytickname=names,/noerase
;        ;,ytitle=ytit
;	xyouts,xpos,ypos0,'time'
;	xyouts,xpos,ypos1,' '+string(time,'(f5.2)')
;	xyouts,xpos,ypos2,run
;	xyouts,xpos,ypos3,'Max='
...
;	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)')
	...
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        ...
        title=' Tn (K)',xstyle=1,ystyle=1,$
        xtitle=' x (km)',ytitle=ytit 
        
	xyouts,xpos,ypos0,'time' 
	xyouts,xpos,ypos1,' '+string(time*2000/65,'(f5.1)') + '  s' 
	xyouts,xpos,ypos2,run 
	xyouts,xpos,ypos3,'Max' 
	xyouts,xpos,ypos4,' '+string(bmax*1000,'(f7.2)')
	xyouts,xpos,ypos5,'Min' 
	xyouts,xpos,ypos6,' '+string(bmin*1000,'(f7.2)')

        fn1=interpolate(vnx,iox,ioy,/grid)
        fn2=interpolate(vny,iox,ioy,/grid)
	...
	xyouts,xpos,ypos2,run
	xyouts,xpos,ypos3,'Max='
	xyouts,xpos,ypos4,'  '+string(fmax*650,'(f5.2)')

  endif

  print, 'continue with next plot, Vnx,Vnz',$
...
; plot 2. page
        ...
        fa=interpolate(vnx,ioxf,ioyf,/grid)
...
;        lvec(16)=mmid+.05
	...
	contour,fa,xf,yf,levels=lvec,$
        c_linestyle=lvec lt 0.0,$ 
        title='Vnx (m/s)',xstyle=1,ystyle=1,$
        xtitle='x (km)',ytitle=ytit,thick=1.0
	xyouts,xpos,ypos0,'time'
	xyouts,xpos,ypos1,' '+string(time*2000/65,'(f5.1)') + '  s'
	xyouts,xpos,ypos2,run
	xyouts,xpos,ypos3,'Max'
	xyouts,xpos,ypos4,' '+string(bmax*650,'(f5.2)')
	xyouts,xpos,ypos5,'Min'
	xyouts,xpos,ypos6,' '+string(bmin*650,'(f5.2)')

        fa=interpolate(vny,ioxf,ioyf,/grid) & f1=smooth(fa,3)
	...
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        ...
        title='Vnz (m/s)',xstyle=1,ystyle=1,$
        xtitle='x (km)', ytitle=ytit
	xyouts,xpos,ypos0,'time'
	xyouts,xpos,ypos1,' '+string(time*2000/65,'(f5.1)') + '  s'
	xyouts,xpos,ypos2,run
...
;        lvec=findgen(13)*del+bmin & lvec(12)=fb
	...
	contour,fa,xf,yf,levels=lvec,$
        ...
        title=' Magnetic Field (nT)',xstyle=1,ystyle=1,$
        xtitle='x (km)',ytitle=ytit
	xyouts,xpos,ypos0,'time'
	xyouts,xpos,ypos1,' '+string(time*2000/65,'(f5.1)') + '  s'
;	xyouts,xpos,ypos2,run 
...
;	xyouts,xpos,ypos2,run
	...
	xyouts,xpos,ypos4,' '+string(fmax,'(f7.4)')
        ...
        fa=interpolate(jz,ioxf,ioyf,/grid) & f1=smooth(fa,3)
	bmax=max(fa) & bmin=min(fa)
        if bmax eq bmin then bmax=bmin+1
        del=(bmax-bmin)/15.
	!P.POSITION=[xa2,ylo,xe2,yup]
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        ...
        title='J_z Component',xstyle=1,ystyle=1,$
        xtitle='x (km)',ytickname=names,/noerase
        ;,ytitle=ytit
	xyouts,xpos,ypos0,'time'
	xyouts,xpos,ypos1,' '+string(time*2000/65,'(f5.1)') + '  s'
	...
	xyouts,xpos,ypos6,' '+string(bmin,'(f5.2)')
        ...
        p=2*u^(5.0/3.0)
        bsq=bsq+p
        fa=interpolate(res,ioxf,ioyf,/grid) & fa=smooth(fa,3) 
	bmax=max(fa)
	bmin=min(fa)
        bav=0.5*(bmax+bmin)
        del=(bmax-bmin)/15.  & if del lt 0.000001 then del=0.000001
	...
	xyouts,xpos,ypos2,run 
	xyouts,xpos,ypos3,'Max' 
	xyouts,xpos,ypos4,' '+string(bmax,'(f6.4)') 
	xyouts,xpos,ypos5,'Min' 
	xyouts,xpos,ypos6,' '+string(bmin,'(f6.4)') 
  endif
...
;	xyouts,xpos,ypos2,run
	xyouts,xpos,ypos3,'Max'
	xyouts,xpos,ypos4,' '+string(fmax*650,'(f5.2)') + '  m/s'
