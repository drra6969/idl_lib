...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   rho=bx & u=bx & rhono=bx & rhono2=bx & rhonn2=bx 
...
;  print, y
   ...
   readu, 8,  vx,vy,vz
   readu, 8,  rho,u,res
   readu, 8,  rescal,ba0,length0,dens0,tempi0,tempe0
;   print, 'rescal = ',rescal,'temp = ',tempe0
   close, 8
...
;        device,/times,/bold,font_index=3
       ...
       !Y.THICK=2
        ...
        if (plane eq 'x') then begin
          ...
          f1=rhon(igrid,*) & f2=pn(igrid,*) & f3=pn(igrid,*)/rhont(igrid,*)
...
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')
  ...
  if (contin eq '' or contin eq 'y') then begin
	...
	!P.POSITION=[xa,ylo1,xe,yup1]
        bmax=max([f17,f19]) & print, 'B max: ',bmax
        bmin=min([f17,f19]) & print, 'B min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
