...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   print, 'dimension nx=',nx,'     ny=',ny,'     nyy=',nyy
	      ...
	      f30=f1 & f31=f1 & f32=f1 & f33=f1
   close, 8

   for it=0,ntmax-1 do begin

; READ INPUT DATA OF DIMENSION NX, NY
	fnumber=string(1+it,form='(i2.2)')
	name='magtap'+fnumber
...
igrid=ix0
          ...
          f20(*,it)=ioniz(igrid,*)
          f30(*,it)=f6300(igrid,*)
          f31(*,it)=n_units*rhono(igrid,*)
          f32(*,it)=n_units*rhono2(igrid,*)/2.
          f33(*,it)=n_units*rhonn2(igrid,*)*4./7.
...
;-----------------------------------------------------------------------
	...
	!P.POSITION=[xa1,ylo,xa2,yup]
        bmax=15.0
        bmin=0.0
        bmax=max([f1]) & print, 'Te max: ',bmax
        bmin=min([f1]) & print, 'Te min: ',bmin
        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
	plot_oi, f1(*,0), plcoo, title='N_n in 1/cm^3', $
	xrange=[1e6,1e14],yrange=[100,700],xstyle=1,ystyle=1,ytitle='z',line=0
        oplot, f31(*,0), plcoo, line=it
        oplot, f32(*,0), plcoo, line=it
        oplot, f33(*,0), plcoo, line=it

	!P.POSITION=[xe2,ylo,xe3,yup]
        bmax=1.e5
        bmin=1.e1
        bmax=max([f5]) & print, 'tn max: ',bmax
        bmin=min([f5]) & print, 'tn min: ',bmin
        delb=bmax-bmin
	...
	plot, f5(*,0), plcoo, title='T_n in 1000 K', $
	xrange=[0,1.5],yrange=[100,700],xstyle=1,ystyle=1,ytitle='z',xticks=3,line=0
...
;	xyouts,xpos,ypos3,plane+' ='+string(x(igrid),'(f6.2)')
  ...
  n2=f33(*,0)
  temp=f5(*,0)*1000.
  save, filename='neutrals', plcoo,o1,o2,n2,temp
