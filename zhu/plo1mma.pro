...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   print, 'dimension nx=',nx,'     ny=',ny,'     nyy=',nyy
	  ...
	  f01=f1 & f02=f1 & f30=f1 & f31=f1 & f32=f1 & f33=f1
   close, 8
...
;        bmin = 180
	...
	plot, plcoo, n_units*f11(*,0), title='Number Density  (10^5 /cm^3)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1, line=0
        for it=1,ntmax-1 do  oplot, plcoo, n_units*f11(*,it), line=it
...
;	xyouts,xpos,/u1/zhu/tempypos2,run
...
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')
	...
	!P.POSITION=[xa,ylo2,xe,yup2]
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, t_units*f13(*,0),title='Ion temperature  (1000 K)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
        for it=1,ntmax-1 do  oplot, plcoo, t_units*f13(*,it), line=it
;	oplot, plcoo, f12, line=1
	...
	!P.POSITION=[xa,ylo3,xe,yup3]
        bmax=max([t_units*f30]) & print, 'Te max: ',bmax
        bmin=min([t_units*f30]) & print, 'Te min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
        ...
        ypos3=bmin+delb/64. 
        ypos4=bmin+delb/128. 
	plot, plcoo, t_units*f30(*,0),title='Electron temperature (1000K)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
        for it=1,ntmax-1 do  oplot, plcoo, t_units*f30(*,it), line=it
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,plane+' ='+string(x(igrid),'(f6.2)')
...
print, 'plot page 2, vy,by,v_vert'
; plot  page 2

	!P.POSITION=[xa,ylo1,xe,yup1]
        bmax=max([vv_units*f15(1:nyy-2,*)]) & print, 'Vy max: ',bmax
        bmin=min([vv_units*f15(1:nyy-2,*)]) & print, 'Vy min: ',bmin
        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
;        bmin = 180
	...
	plot, plcoo, vv_units*f15(*,0), title='Vertical Velocity (m/s)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1, line=0
        for it=1,ntmax-1 do  oplot, plcoo, vv_units*f15(*,it), line=it
;	oplot, plcoo, f2, line=1
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos4,plane+' ='+string(x(igrid),'(f6.2)')
...
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')
	...
	plot, plcoo, v_units*f16(*,0),title='Horizontal velocity Vy (km/s)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
        for it=1,ntmax-1 do  oplot, plcoo, v_units*f16(*,it), line=it
;	oplot, plcoo, f12, line=1
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,plane+' ='+string(x(igrid),'(f6.2)')
...
;        bmax=10000.0 & bmin=1850.0
	...
	plot, plcoo, b_units*f19(*,0),title='Magnetic Field By (nT)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
        for it=1,ntmax-1 do  oplot, plcoo, b_units*f19(*,it), line=it
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,plane+' ='+string(x(igrid),'(f6.2)')
...
print, 'plot page 3, jy,ey,res'
; plot  page 2

	!P.POSITION=[xa,ylo1,xe,yup1]
        bmax=max([curr_units*f01(1:nyy-2,*)]) & print, 'Jy max: ',bmax
        bmin=min([curr_units*f01(1:nyy-2,*)]) & print, 'Jy min: ',bmin
        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
;        bmin = 180
	...
	plot, plcoo, curr_units*f01(*,0), $
         title='Upward Current (uA/m^2)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1, line=0
        for it=1,ntmax-1 do  oplot, plcoo, curr_units*f01(*,it), line=it
;	oplot, plcoo, f2, line=1
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos4,plane+' ='+string(x(igrid),'(f6.2)')
...
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')
	...
	!P.POSITION=[xa,ylo2,xe,yup2]
        bmax=max([e_units*f02]) & print, 'Ey max: ',bmax
;        bmin=min([e_units*f02]) & print, 'Ey min: ',bmin
        bmin=-20.0
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, e_units*f02(*,0),title='Vertical Electric Field (uV/m)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
        for it=1,ntmax-1 do  oplot, plcoo, e_units*f02(*,it), line=it
;	oplot, plcoo, f12, line=1
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,plane+' ='+string(x(igrid),'(f6.2)')
...
save,file='realnam2.dat',/xdr,f01,f02,f7,f13,plcoo
end
