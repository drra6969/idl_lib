...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   for j=0,nx-1 do srho(j,*)=smooth(rho(j,*),5)
...
;   ez=nuen*vz*lambdam-vex*by+vey*bx+res*jz
   ex=res*jx+nuen*vx*lambdam+ex_dpe-vey*bz+vez*by
   ey=res*jy+nuen*vy*lambdam
   ez=res*jz+nuen*vz*lambdam-vex*by+vey*bx
...
;   for j=0,nx-1 do begin
;      for k=0,40 do begin
;        jxs(j) = jxs(j) + (jx(j,k)+jx(j,k+1))/2.0*(y(k+1)-y(k)) $
;                         *j_units*l_units                     ; milli A /m
;        jzs(j) = jzs(j) + (jz(j,k)+jz(j,k+1))/2.0*(y(k+1)-y(k)) $
;                         *j_units*l_units                     ; milli A /m
;        jps(j) = jps(j) + (jp(j,k)+jp(j,k+1))/2.0*(y(k+1)-y(k)) $
;                         *j_units*l_units                     ; milli A /m
;        jhs(j) = jhs(j) + (jh(j,k)+jh(j,k+1))/2.0*(y(k+1)-y(k)) $
;                         *j_units*l_units                     ; milli A /m
;      endfor
;   endfor
;    name = 'cut'+fnumber
...
;        bmin=0.0
  ...
  print, 'plot 2. page, high integrated current?'
  read, contin
  if (contin eq '' or contin eq 'y') then begin
;-----------------------------------------------------------------------
;    plot page 2 ( Vn, V, Ve)
...
;        bmax=max([f4]) & print, 'Vn max: ',bmax
;        bmin=min([f4]) & print, 'Vn min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
...
;        bmax=max([f14a,-f16a]) & print, 'Ve max: ',bmax
;        bmin=min([f14a,-f16a]) & print, 'Ve min: ',bmin
;        bmax=0.45
;        bmin=-0.45
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
...
;-----------------------------------------------------------------------
  ...
  if (contin eq '' or contin eq 'y') then begin
	...
	!P.POSITION=[xa,ylo2,xe,yup2]
        bmax=max([f05]) & print, 'J max: ',bmax
        bmin=min([f05]) & print, 'Jy min: ',bmin
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
        ypos0=bmin+0.9*delb    
        ypos1=bmin+0.82*delb 
...
print, 'plot page 6, sigmap,sigmah,rho'

;-----------------------------------------------------------------------
; plot  page 6 ( Sigmap, Sigmah, rho, )
;-----------------------------------------------------------------------
...
;        bmin=min([fpe]) & print, 'Ve min: ',bmin
        bmax=max([f11]) & print, 'Ve max: ',bmax
        bmin=min([f11]) & print, 'Ve min: ',bmin
;        bmax=2.5e5
;        bmin=1.5e4
        delb=bmax-bmin
        if (bmin eq bmax) then bmax=bmin+1.0
...
;	plot, plcoo, f14a,title=' Pe (nPa) ', $
	plot_io, plcoo, f11,title=' Plasma Number Density ', $
	yrange=[bmin,bmax],xstyle=1,ystyle=1
;	oplot, plcoo, f15a, line=1
;	oplot, plcoo, f16a, line=2
	xyouts,xpos,ypos0,'time:'
