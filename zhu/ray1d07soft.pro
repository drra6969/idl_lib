...
;--------------------
   ...
   gradp=bx & gradpe=bx & gradpex=bx & gradpn=bx
...
; parameters for red line volume emission
   ...
   close, 8
   vnx=smooth(vnx/rhon,5) &  vnz=smooth(vnz/rhon,5) & vny=smooth(vny/rhon,5)
   ...
   tna = pn1/rhont & tn=pn/rhont & tia=tna & tea=p1/rho1-tia
     ...
     vny = vny
      ...
      gradpn=(shift(pn,0,-1)-shift(pn,0,1))
      for j=1,ny-2 do gradp(*,j)=dy(j)*gradp(*,j)
      for j=1,ny-2 do gradpn(*,j)=dy(j)*gradpn(*,j)
;     gradpn1=smooth(gradpn,3) &gradpn=gradpn1
...
;          f6(*,it) = t_units*ti(igrid,*)
          ...
          f36(*,it)= vv_units/ta_time*(-0.5*gradpn(igrid,*)/rhon(igrid,*)-gravit(*))

          f8(*,it) = n_units*rhono(igrid,*)
          f9(*,it) = n_units*rhono2(igrid,*)
          f10(*,it)= n_units*rhonn2(igrid,*)
          f14(*,it)= j_units*jy(igrid,*)
          f16(*,it) = t_units*tn(igrid,*)
          f17(*,it) = t_units*ti(igrid,*)
          f18(*,it) = n_units*rhont(igrid,*)
          f36(*,it)=smooth(f36(*,it),5)
...
;-----------------------------------------------------------------------
; 4th page ( density and neutral)
;-----------------------------------------------------------------------

	!P.POSITION=[xa1,ylo,xe1,yup]
        bmax=2.e6 & print, 'rho max: ',bmax
        bmin=1.e4 & print, 'rho min: ',bmin
...
;        bmin=min([f35]) & print, 'Vel min: ',bmin
        ...
        ypos1=pmin+0.4*delb 
        ypos3=pmin+0.26*delb 
	plot, f35(*,0), plcoo, title='Acceleration Terms', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
...
;-----------------------------------------------------------------------
; 6th page (  neutral force density and velocity)
;-----------------------------------------------------------------------
...
;        bmin=min([f35]) & print, 'Vel min: ',bmin
        ...
        ypos1=pmin+0.4*delb 
        ypos3=pmin+0.26*delb 
	plot, f36(*,0), plcoo, title='Neutral Acceleration', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
        for it=1,ntmax-1 do  oplot, f36(*,it), plcoo, line=it
	xyouts,xpos,ypos0,charsize=1,'time:'
	xyouts,xpos,ypos1,charsize=1,' '+string(time*ta_time,'(f6.2)')+ ' s'
	xyouts,xpos,ypos3,charsize=1,plane+' ='+string(x(igrid),'(f6.2)')
...
;-----------------------------------------------------------------------
; 7th page (  neutral density and temperature)
;-----------------------------------------------------------------------
	...
	!P.POSITION=[xa1,ylo,xe1,yup]
        ...
        ypos1=pmin+0.4*delb 
        ypos3=pmin+0.26*delb 
	plot, f18(*,0), plcoo, title='Neutral Density', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0,/xlog
        for it=1,ntmax-1 do  oplot, f18(*,it), plcoo, line=it
        oplot, f8(*,0), plcoo,line=0
...
;        bmin=min([f35]) & print, 'Vel min: ',bmin
        ...
        ypos1=pmin+0.4*delb 
        ypos3=pmin+0.26*delb 
	plot, 1000*(f16(*,0)-f16(*,0)), plcoo, title='Neutral Temperature Change (K)', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
        for it=1,ntmax-1 do  oplot, 1000*(f16(*,it)-f16(*,0)), plcoo, line=it
	xyouts,xpos,ypos0,charsize=1,'time:'
	xyouts,xpos,ypos1,charsize=1,' '+string(time*ta_time,'(f6.2)')+ ' s'
	xyouts,xpos,ypos3,charsize=1,plane+' ='+string(x(igrid),'(f6.2)')
