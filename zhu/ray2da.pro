...
; parameters for red line volume emission
   ...
   readu, 8,  x,dx,g2,g3,g3,g3,g3, y,dy,h2,h3,h3,h3,h3
   readu, 8,  vnx,vny,vnz
   readu, 8,  rhono,rhono2,rhonn2,rhon,un,rhona,una,nu12s,nu12a,ioniz
   close, 8
...
;   for i=1,nx-2 do gpx(i,*)=dx(i)*f1(i,*)
   f1=shift(pe,0,-1)-shift(pe,0,1)
      for i=1,ny-2 do gradpe(*,i)=dy(i)*f1(*,i)
      for i=0,nx-1 do gradpe(i,0)=0.00001 
      for i=0,nx-1 do gradpe(i,ny-1)=0.00001
   f1=shift(pe,-1,0)-shift(pe,1,0)
      for i=1,nx-2 do gradpex(i,*)=dx(i)*f1(i,*)
...
;        !X.TICKS=2
;        bmax=max([fvy]) & print, 'vy max: ',bmax
;        bmin=min([fvy]) & print, 'vy min: ',bmin
;        bmin=-2.5
;        delb=pmax-pmin
...
;	plot, fvy, plcoo, $
;	title='Vy (km/s) at x=0 ', $
;	xrange=[bmin,bmax],xstyle=1,xtitle='Vy',ystyle=1,ytitle='z',line=0
;	xyouts,xpos,ypos0,'time:'
;	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.3)') + ' s'
;;	xyouts,xpos,ypos3,'x ='+string(x(39),'(f6.2)')
  endif
        !X.TICKS=4

  print, 'continue with next page? Red line, temperature'
  read, contin
  if (contin eq '' or contin eq 'y') then begin
...
;       contplot, ff,xa1,xe1,ylo,yup,'(f7.2)',xtit,tit,tt,15,1
       ...
       ff = -f6300
;       tt = ta_time*time
       contplot, ff,xa1,xe1,ylo,yup,'(f9.2)',xtit,tit,tt,15,1
...
;       tt = ta_time*time
       ...
       ff = -t_units*(te-tea)
;       tt = ta_time*time
       contplot, ff,xa3,xe3,ylo,yup,'(E8.1)',xtit,tit,tt,11,1
...
;       tt = ta_time*time
       ...
       ff = -j_units*jy
;       tt = ta_time*time
       contplot, ff,xa3,xe3,ylo,yup,'(f7.2)',xtit,tit,tt,16,1
