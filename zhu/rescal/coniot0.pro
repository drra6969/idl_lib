...
;        fa=interpolate(tempn,ioxf,ioyf,/grid)
	...
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        ...
        title=' Upward Current Density (uA/m^2)', $
        xstyle=1,ystyle=1,$
        xtitle=' x (km)',ytitle=ytit 
...
;---------------------------------------------------------------
	...
	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)') 
        ...
        f1=smooth(jx,3) & fa=interpolate(f1,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
        if (bmax-bmin) lt 0.00001 then bmax=bmin+1.
	del=(bmax-bmin)/15.
...
;        !P.CHARSIZE=2.0
	...
	contour,fa,xf,yf,levels=findgen(11)*del+bmin,$
        c_linestyle=findgen(11)*del+bmin lt 0.0,$
        title=' Current Density, JY',xstyle=1,ystyle=1,$
        xtitle=' x',ytitle=ytit 
	...
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        c_linestyle=findgen(16)*del+bmin lt 0.0,$
        title=' Electric Field, EY',xstyle=1,ystyle=1,$
        xtitle=' x',ytitle=ytit 
	...
	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)') 
        ...
        f1=smooth(ex,3) & fa=interpolate(f1,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
         print,bmax,bmin
        if (bmax-bmin) lt 0.00001 then bmax=bmin+1.
...
;        fa=interpolate(f2,ioxf,ioyf,/grid)
;      surface,fb,xf,yf,position=[0.2,0.1,0.45,1.1,0.1,0.5],ax=35,$
;      ztitle='Current Density JY',xstyle=1,ystyle=1,$
;                                xtitle='x',ytitle=ytit
;      surface,fa,xf,yf,position=[0.6,0.2,0.85,1.2,0.4,0.9],ax=35,$
;      ztitle='Electric Field EY',xstyle=1,ystyle=1,$
