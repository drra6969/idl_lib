...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   readu, 8,  nx,ny,time
   print, 'dimension nx=',nx,'     ny=',ny, 'time=', time
...
;        vect, fn1, fn2, xn, yn, length=5.*fmax,$
        vect, fn1, fn2, xn, yn, length=fmax,$
...
;        fb=smooth(fb,3)
    ...
    surface,fa,xf,yf,position=[0.6,0.2,0.85,1.2,0.4,0.9],ax=35,$
      ztitle='ptot',xstyle=1,ystyle=1,xtitle='x',ytitle=ytit,/noerase

  endif

  print, 'continue with next plot, bn, and vy'
  read, contin
  if (contin eq '' or contin eq 'y') then begin
...
; plot 5. page
	...
	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)') 
        ...
        f1=smooth(vy,3) & fa=interpolate(f1,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
        del=(bmax-bmin)/4.
	!P.POSITION=[xa2,ylo,xe2,yup]
	contour,fa,xf,yf,levels=findgen(5)*del+bmin,$
        c_linestyle=findgen(5)*del+bmin gt 0.0,$
        title=' Vy Component',xstyle=1,ystyle=1,$
        xtitle=' x',ytitle=ytit 
	...
	xyouts,xpos,ypos6,'  '+string(bmin,'(f5.2)') 
        ...
        f1=smooth(by,3) & fa=interpolate(f1,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
        del=(bmax-bmin)/4.
	!P.POSITION=[xa3,ylo,xe3,yup]
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        c_linestyle=findgen(5)*del+bmin lt 0.0,$
        title=' By ',xstyle=1,ystyle=1,$
        xtitle=' x',ytitle=ytit 
...
; plot 7. page
        ...
        f1=smooth(jz,3) & fa=interpolate(jz,ioxf,ioyf,/grid)
	bmax=max(fa)
	bmin=min(fa)
        del=(bmax-bmin)/15.
	!P.POSITION=[xa1,ylo,xe1,yup]
	contour,fa,xf,yf,levels=findgen(16)*del+bmin,$
        c_linestyle=findgen(16)*del+bmin gt -1.0,$
