...
;       tit = ' Density !9 r !X (1/cm^3)'
       ...
       ff = -n_units*rho
;       tt = ta_time*time
       contplot, ff,xa1,xe1,ylo,yup,'(E8.1)',xtit,tit,tt,11,1
...
;        fmax=max(sqrt(fn1^2+fn2^2))
	...
	!P.POSITION=[xa1,ylo,xe1,yup]
        vect, fn1*5.5, fn2, xn, yn, length=1.5,$
	...
	!P.POSITION=[xa2,ylo,xe2,yup]
        vect, fn1*2.5, fn2, xn, yn, length=1.0,$
...
;---------------------------------------------------------------
       ...
       ff = -e_units*ex*0.001
;       tt = ta_time*time
       contplot, ff,xa1,xe1,ylo,yup,'(f7.2)',xtit,tit,tt,17,1
...
;       tt = ta_time*time
       ...
       ff = -n_units*rho
;       tt = ta_time*time
       contplot, ff,xa3,xe3,ylo,yup,'(E8.1)',xtit,tit,tt,21,1
...
;---------------------------------------------------------------
       ...
       ff = -jx*j_units
;       tt = ta_time*time
       contplot, ff,xa1,xa2,ylo,yup,'(f7.2)',xtit,tit,tt,10,1

       tit = ' Jy (uA/m^2)'
       xtit = ' '
       ff = jz*j_units
;       tt = ta_time*time
       contplot, ff,xe2,xe3,ylo,yup,'(f7.2)',xtit,tit,tt,11,1
...
;       contplot, ff,xa3,xe3,ylo,yup,'(f7.2)',xtit,tit,tt,10,1
  ...
  print, 'continue with next plot, Conductivities'
  read, contin
  if (contin eq '' or contin eq 'y') then begin
