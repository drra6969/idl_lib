...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   print, 'dimension nx=',nx,'     ny=',ny,'     nyy=',nyy
	  ...
	  f01=f1 & f02=f1 & fdt=f1 & fdv2=f1 & fratio=f1 & fnuin=f1 & fnuen=f1 & fnuei=f1
	  ftime=fltarr(ntmax+1, /NOZERO)
   close, 8
...
;          plcoo=y(0:ny-1) 
	  ...
	  f01(*,it)=jy(igrid,*) & f02(*,it)=ey(igrid,*)
          ...
          f4(*,it)=vnx(igrid,*) & f5(*,it)=vny(igrid,*) & f6(*,it)=vnz(igrid,*)
...
;          f21(*,it)=pn(*,igrid)/rhont(*,igrid)-p(*,igrid)/rho(*,igrid)
          ...
          fdt=f13-f3
          fdv2=(f14-f4)*(f14-f4)+(f15-f5)*(f15-f5)+(f16-f6)*(f16-f6)
          fratio=fdt/fdv2
...
; plot first page
	...
	!P.POSITION=[xa,ylo1,xe,yup1]
        bmax=t_units*max([fdt(yindex,*)]) & print, 'Jy max: ',bmax
        bmin=t_units*min([fdt(yindex,*)]) & print, 'Jy min: ',bmin
...
;        bmin = 180
	...
	plot, plcoo, t_units*fdt(yindex,*), title='Ti-Tn  (1000 K)', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1, line=0
	xyouts,xpos,ypos1,' '+string( ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos4,'y'+' ='+string(y(yindex),'(f6.2)')
;	xyouts,xpos,ypos2,run
;	xyouts,xpos,ypos3,'Max='
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')

	!P.POSITION=[xa,ylo2,xe,yup2]
        bmax=v_units*v_units*max([fdv2(yindex,*)]) & print, 'Ey max: ',bmax
        bmin=v_units*v_units*min([fdv2(yindex,*)]) & print, 'Ey min: ',bmin
        delb=bmax-bmin
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, v_units*v_units*fdv2(yindex,*),title='(vi-vn)^2', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,'y'+' ='+string(y(yindex),'(f6.2)')

	!P.POSITION=[xa,ylo3,xe,yup3]
;        bmax=res_units*max([f7]) & print, 'Res max: ',bmax
;        bmin=res_units*min([f7]) & print, 'Res min: ',bmin
        bmax=max([fratio(yindex,5:40)]) & print, 'Te max: ',bmax
        bmin=min([fratio(yindex,5:40)]) & print, 'Te min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
        ...
        ypos3=bmin+delb/64. 
        ypos4=bmin+delb/128. 
	plot, plcoo, fratio(yindex,*),title='(Ti-Tn)/(vi-vn)^2', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
...
; plot first page
	...
	!P.POSITION=[xa,ylo1,xe,yup1]
        bmax=t_units*max([fnuin(yindex,*)]) & print, 'Jy max: ',bmax
        bmin=t_units*min([fnuin(yindex,*)]) & print, 'Jy min: ',bmin
        if (bmax-bmin) lt 0.0001 then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
;        bmin = 180
	...
	plot, plcoo, fnuin(yindex,*), title='nuin', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1, line=0
	xyouts,xpos,ypos1,' '+string( ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos4,'y'+' ='+string(y(yindex),'(f6.2)')
;	xyouts,xpos,ypos2,run
...
;	xyouts,xpos,ypos4,'  '+string(fmax,'(f7.4)')
	...
	!P.POSITION=[xa,ylo2,xe,yup2]
        bmax=n_units*max([f11(yindex,*)]) & print, 'Ey max: ',bmax
        bmin=n_units*min([f11(yindex,*)]) & print, 'Ey min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, n_units*f11(yindex,*),title='rho', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,'y'+' ='+string(y(yindex),'(f6.2)')
...
;        bmax=res_units*max([f7]) & print, 'Res max: ',bmax
;        bmin=res_units*min([f7]) & print, 'Res min: ',bmin
        bmax=max([fnuei(yindex,*)]) & print, 'Te max: ',bmax
        bmin=min([fnuei(yindex,*)]) & print, 'Te min: ',bmin
        delb=bmax-bmin
        if bmin eq bmax then bmax=bmin+1.0
        delb=bmax-bmin
        xpos=pmax+0.015*(pmax-pmin)
;        bmax=10000.0 & bmin=1850.0
        ...
        ypos3=bmin+delb/64. 
        ypos4=bmin+delb/128. 
	plot, plcoo, fnuei(yindex,*),title='nuei', $
	  yrange=[bmin,bmax],xstyle=1,ystyle=1
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.2)')+'s'
	xyouts,xpos,ypos3,'y'+' ='+string(y(yindex),'(f6.2)')
...
save,file='/why1/zhu/io2d/alfven/data/rot/out.dat',$
