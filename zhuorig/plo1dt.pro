...
; READ INPUT DATA OF DIMENSION NX, NY
   ...
   rhon=bx & un=bx & rhon1=bx & un1=bx & fdt=bx & fdv2=bx
...
;   for j=0,nx-1 do te(j,*)=smooth(te(j,*),7)
;   for j=0,nx-1 do rho(j,*)=smooth(rho(j,*),7)
;   for j=0,nx-1 do jy(j,*)=smooth(jy(j,*),7)
...
gridindex:
  ...
  if (igrid lt 0) or (igrid gt nplane-1) then igrid=(nplane+1)/2  
   ...
   fdt=(ti-tna)*t_units
   fdv2=0.64674*meff*v_units*v_units*4.0 $
         *((vx-vnx)*(vx-vnx)+(vy-vny)*(vy-vny)+(vz-vnz)*(vz-vnz))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Hight integrated conductivities
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   for i=0, nx-1 do begin
      ssigmap(i)=0.0
      ssigmah(i)=0.0
      for j=0, ny-2 do begin
        ssigmap(i)=ssigmap(i)+sigmap(i,j)/dy(j)
        ssigmah(i)=ssigmah(i)+sigmah(i,j)/dy(j)
      endfor
      ssigmap(i)=ssigmap(i)*l_units*1000.0
      ssigmah(i)=ssigmah(i)*l_units*1000.0
   endfor
   ssigmaa=1.0/vv_units*rescal/4.0/3.14159*1.0e7
   print, 'sa = ', ssigmaa
   print, 'sp = ', ssigmap(39)
;    name = 'cut'+fnumber
;    openw, 8, name
...
;        device,/times,/bold,font_index=3
	  ...
	  fvtex=y & fvtey=y & fvtez=y & fne=y & fni=y & fni20=y & fni39=y
	  f40=y & f41=y & f42=y & f43=y & f44=y & f50=y
	  f60=y & f61=y & f62=y & f63=y & f64=y & f65=y
	  f70=y & f71=y & f75=y & f76=y
	  f80=y & f81=y & f82=y & f83=y
	  f90=y & f91=y & f92=y & f93=y
	  ...
	  fne=nuen(igrid,*)/ta_time & fni=nuin(igrid,*)/ta_time
;	  fni20=nuin(20,*)/ta_time & fni39=nuin(39,*)/ta_time 
          ...
          f21=t_units*(pn(igrid,*)/rhont(igrid,*)-ti(igrid,*))
          ...
          f75=fdt(igrid,*) & f76=fdv2(igrid,*)
          f80=-nuen(igrid,*)*vx(igrid,*)*lambdam*e_units
          f81=(-vey(igrid,*)*bz(igrid,*)+vez(igrid,*)*by(igrid,*))*e_units
          f82=res(igrid,*)*jx(igrid,*)*e_units
          f83=ex_dpe(igrid,*)*e_units
          f90=nuen(igrid,*)*vy(igrid,*)*lambdam*e_units
          f91=(-vez(igrid,*)*bx(igrid,*)+vex(igrid,*)*bz(igrid,*))*e_units
          f92=res(igrid,*)*jy(igrid,*)*e_units
          f93=ey_dpe(igrid,*)*e_units
        endif
        if (plane eq 'y') then begin
...
print, 'plot page 2, Jz and Te'

;-----------------------------------------------------------------------
; plot  page 2 ( Jz and Te )
;-----------------------------------------------------------------------
...
;        bmax=max([f60,f63,f64]) & print, 'Rho max: ',bmax
;        bmin=min([f60,f63,f64]) & print, 'Rho min: ',bmin
;        bmax=max([f23]) & print, 'Rho max: ',bmax
;        bmin=min([f23]) & print, 'Rho min: ',bmin
        bmax = 15.0
        bmin = 0.0
        delb=bmax-bmin
;        if bmin eq bmax then bmin=bmin*.9 & bmax=bmax*1.1
	...
	plot, f23,plcoo, $
;	title='Ez, Ez_res, Ez_gradpe, Ez_iner, Ez_g (uv/m)', $
	title='Electron Temperature (1000 K)', $
;	title='By (nT)', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
;	oplot, f61, plcoo, line=1
;	oplot, f62, plcoo, line=2
...
;-----------------------------------------------------------------------
  ...
  if (contin eq '' or contin eq 'y') then begin
	...
	!P.POSITION=[xa1,ylo,xa2,yup]
        bmax=max([f04]) & print, 'Ex max: ',bmax
        bmin=min([f04]) & print, 'Ex min: ',bmin
;        bmax=700.0
;        bmin=0.0
        delb=bmax-bmin
        if (bmin eq bmax) then begin 
...
;	title='Ez, Ez_res, Ez_gradpe, Ez_iner, Ez_g (uv/m)', $
	...
	!P.POSITION=[xe2,ylo,xe3,yup]
;        bmax=max([-f06]) & print, 'Ey max: ',bmax
;        bmin=min([-f06]) & print, 'Ey min: ',bmin
        bmax = 100.0
        bmin = -100.0
        delb=bmax-bmin
        delb=pmax-pmin
...
print, 'plot page 4, Ionization Rate'
...
;	title='nuin 1/s', $
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
;	oplot, f04, plcoo, line=0
	oplot, f41*1.e6, plcoo, line=1
	oplot, (f42+1.0e-10)*1.e6, plcoo, line=2
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.3)') + ' s'
	xyouts,xpos,ypos3,plane+' ='+string(coord(igrid),'(f6.2)')
...
print, 'plot page 4, Terms in the electric field'
...
;	title='Ionization Rate (1/cm^3/s)', $
	...
	xrange=[bmin,bmax],xstyle=1,ystyle=1,ytitle='z',line=0
;	oplot, f04, plcoo, line=0
	oplot, f76, plcoo, line=1
;	oplot, f80/1000., plcoo, line=1
;	oplot, f81/1000., plcoo, line=2
;	oplot, f82/1000., plcoo, line=3
;	oplot, f83/1000., plcoo, line=4
	xyouts,xpos,ypos0,'time:'
	xyouts,xpos,ypos1,' '+string(ta_time*time,'(f6.3)') + ' s'
	xyouts,xpos,ypos3,plane+' ='+string(coord(igrid),'(f6.2)')
