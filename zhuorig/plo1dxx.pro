...
; READ INPUT DATA OF DIMENSION NX, NY
   print, 'Input filenumber for ion temperature'
   read, fnumber
   name='magtap'+fnumber
   ...
   print, 'Input filenumber for electron temperature'
   read, fnumber
   name='magtap'+fnumber
   openr, 8, name,/F77_UNFORMATTED
   readu, 8,  nx,ny,nyy,time
...
;        device,/times,/bold,font_index=3
       ...
       !Y.THICK=2
          ...
          tn1=x & tn2=x & tn3=x & tn4=x & tn5=x
          ne1=x & ne2=x & ne3=x & ne4=x & ne5=x  
          ti1=t_units*ti(*,igrid1)
          te1=t_units*te(*,igrid1)
          tn1=t_units*tn(*,igrid1)
          ne1=n_units*rho(*,igrid1)
          ti2=t_units*ti(*,igrid2)
          te2=t_units*te(*,igrid2)
          tn2=t_units*tn(*,igrid2)
          ne2=n_units*rho(*,igrid2)
          ti3=t_units*ti(*,igrid3)
          te3=t_units*te(*,igrid3)
          tn3=t_units*tn(*,igrid3)
          ne3=n_units*rho(*,igrid3)
          ti4=t_units*ti(*,igrid4)
          te4=t_units*te(*,igrid4)
          tn4=t_units*tn(*,igrid4)
          ne4=n_units*rho(*,igrid4)
          ti5=t_units*ti(*,igrid5)
          te5=t_units*te(*,igrid5)
          tn5=t_units*tn(*,igrid5)
          ne5=n_units*rho(*,igrid5)
          

        !X.RANGE=[pmin,pmax]
;        !X.RANGE=[2,4]
          hight1 = 'z = '+strtrim(string(long(y(igrid1))),2) + ' km'
          hight2 = 'z = '+strtrim(string(long(y(igrid2))),2) + ' km'
          hight3 = 'z = '+strtrim(string(long(y(igrid3))),2) + ' km'
...
;        bmax=max([ne1,ne2,ne3]) & print, 'Ti max: ',bmax
        bmax=250000.0
;        bmin=min([ne1,ne2,ne3]) & print, 'Ti min: ',bmin
        bmin=50000.0
        ...
        ypos4=bmin+0.3*delb 
	plot, plcoo, ne1, TITLE=  'Electron density', $
	ytitle=' ne (1/cm^3)',yrange=[bmin,bmax],xstyle=1,ystyle=1
	oplot, plcoo, ne2, line=1
	oplot, plcoo, ne3, line=2
	oplot, [-11.5,-10],[4.7e5,4.7e5],line=2
	xyouts,-9.7,4.67e5,hight3
	oplot, [-11.5,-10],[4.4e5,4.4e5],line=1
	xyouts,-9.7,4.37e5,hight2
	oplot, [-11.5,-10],[4.1e5,4.1e5]
	xyouts,-9.7,4.07e5,hight1	
	xyouts,xpos,ypos2,'time:'
	xyouts,xpos,ypos3,' '+string(timee*ta_time,'(f6.2)')+ ' s'
...
;        bmin=min([ti1,ti2,ti3]) & print, 'Ti min: ',bmin
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, ti1, title='Ion temperature', $
	ytitle=' Ti (1000 K)',yrange=[bmin,bmax],xstyle=1,ystyle=1
	oplot, plcoo, ti2, line=1
	oplot, plcoo, ti3, line=2
	oplot, plcoo, tn1, line=3
;	oplot, plcoo, tn2, line=4
;	oplot, plcoo, tn3, line=5
	oplot, [-4.5,-3],[2.3,2.3],line=3
	xyouts,-2.7,2.26,'Tn at '+strtrim(string(long(y(igrid1))),2) + ' km'
	oplot, [-11.5,-10],[2.3,2.3],line=2
	xyouts,-9.7,2.26,hight3
	oplot, [-11.5,-10],[2.15,2.15],line=1
	xyouts,-9.7,2.11,hight2
	oplot, [-11.5,-10],[2.0,2.0]
	xyouts,-9.7,1.95,hight1	
	xyouts,xpos,ypos2,'time:'
	xyouts,xpos,ypos3,' '+string(timei*ta_time,'(f6.2)')+ ' s'
;	xyouts,xpos,ypos3,plane+' ='+string(coord(igrid),'(f6.2)')
...
;        bmin=min([te1,te2,te3]) & print, 'Te min: ',bmin
        ...
        ypos3=bmin+0.56*delb 
        ypos4=bmin+0.3*delb 
	plot, plcoo, te1,title='Electron temperature', $
	xtitle=' x (km)',ytitle=' Te (1000 K)',$
...
;	oplot, plcoo, tn2, line=4
;	oplot, plcoo, tn3, line=5
	oplot, [-4.5,-3],[2.8,2.8],line=3
	xyouts,-2.7,2.75,'Tn at '+strtrim(string(long(y(igrid1))),2) + ' km'
	oplot, [-11.5,-10],[2.8,2.8],line=2
	xyouts,-9.7,2.75,hight3
	oplot, [-11.5,-10],[2.55,2.55],line=1
	xyouts,-9.7,2.5,hight2
	oplot, [-11.5,-10],[2.3,2.3]
	xyouts,-9.7,2.25,hight1	
	xyouts,xpos,ypos2,'time:'
	xyouts,xpos,ypos3,' '+string(timee*ta_time,'(f6.2)')+ ' s'
;	xyouts,xpos,ypos3,plane+' ='+string(coord(igrid),'(f6.2)')
