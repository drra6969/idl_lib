; START OF MAIN PROGRAM
  ...
  while not eof(8)  do begin
    ...
    fexarr(*,itime) = T0 &  rms(itime) = error
    itot=itime
   print,'itime = ', itime
...
;      amin=-0.0 & amax = 1.5
      ...
      i=itot 
       !P.THICK=3.      
       !P.POSITION=[0.2,0.35,0.8,0.65] 
        plot, x, farr(*,0),$
          title='Temperature',xtitle='X',$
          xstyle=1, ystyle=1, line=2
       !P.THICK=1.      
        oplot, x, farr(*,i), line=1
        oplot, x, fexarr(*,1), line=0
         xpos=1.02*xmax
...
;	 xyouts,xpos,ypos0,run
	 xyouts,xpos,ypos0,'time='+string(t(i),'(f5.2)')
	 xyouts,xpos,ypos,'u='+string(u,'(f6.3)')
	 xyouts,xpos,ypos1,'nu='+string(nu,'(f6.3)')
	 xyouts,xpos,ypos2,'dt='+string(dt,'(f6.4)')
